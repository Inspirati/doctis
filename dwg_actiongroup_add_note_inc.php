 ...
 * @return void
 */
function dwg_action_add_note_print_title() {
	echo lang_get( 'add_dwgnote_title' );
}
 ...
 * @return void
 */
function dwg_action_add_note_print_fields() {
?>
	<tr>
		<th class="category">
	...
	<tr>
		...
		<td>
<?php
	$t_default_state = config_get( 'default_dwgnote_view_status' );
	if( access_has_project_level( config_get( 'set_view_status_threshold' ) ) ) { ?>
				<select name="view_state" class="input-sm">
					<?php print_dwg_enum_string_option_list( 'view_state', $t_default_state ) ?>
				</select>
<?php
	} else {
...
function dwg_action_add_note_validate( $p_bug_id ) {
	...
	$t_add_bugnote_threshold = config_get( 'add_dwgnote_threshold' );
	$t_bug_id = $p_bug_id;

	if( dwg_is_readonly( $t_bug_id ) ) {
		return lang_get( 'actiongroup_error_document_is_readonly' );
	}

	if( !access_has_dwg_level( $t_add_bugnote_threshold, $t_bug_id ) ) {
