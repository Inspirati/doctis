 ...
 */
function dwg_revision_add( $p_bug_id, $p_user_id, $p_type, $p_value, $p_bugnote_id = 0, $p_timestamp = null ) {
	if( $p_type <= REV_ANY ) {
		return null;
	}

	$t_last = dwg_revision_last( $p_bug_id, $p_type );
...
function dwg_revision_get( $p_revision_id ) {
	...
	if( !$t_row ) {
		trigger_error( ERROR_DWG_REVISION_NOT_FOUND, ERROR );
	}
 ...
 */
function dwg_revision_get_type_name( $p_revision_type_id ) {
	$t_type_name = '';
 ...
 * @return void
 */
function dwg_revision_drop( $p_revision_id ) {
	db_param_push();
	...
	if( is_array( $p_revision_id ) ) {
		...
		foreach( $p_revision_id as $t_rev_id ) {
			...
			$t_revisions[$t_rev_id] = dwg_revision_get( $t_rev_id );
		}
