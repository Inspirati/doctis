...
function email_template_bug_message( $issue_data,$top_line ) {
	...
	$variables['severity'] = get_enum_element( 'severity', $issue_data['email_severity'], auth_get_current_user_id(), $issue_data['email_project_id']  );
	$variables['status'] = get_enum_element( 'status', $issue_data['email_status'], auth_get_current_user_id(), $issue_data['email_project_id'] );
	$variables['priority'] = get_enum_element( 'priority', $issue_data['email_priority'], auth_get_current_user_id(),$issue_data['email_project_id'] );

	if ( isset( $issue_data[ 'email_reproducibility' ] ) ) {
		$variables['repro'] = get_enum_element( 'reproducibility', $issue_data['email_reproducibility'], auth_get_current_user_id(), $issue_data['email_project_id'] );
	} else {
		$variables['repro'] = 0;
	}
	...
	if ( isset( $issue_data[ 'email_steps_to_reproduce' ] ) ) {
		$variables['steps'] = $issue_data['email_steps_to_reproduce'];
	} else {
		$variables['steps'] = 0;
	}
	if ( isset( $issue_data[ 'email_due_date' ] ) ) {
		$variables['due_date'] = $issue_data['email_due_date'];
	} else {
		$variables['due_date'] = 0;
	}
...
if (plugin_is_loaded('Defcon')){
	$t_bug_id = 	$variables['bug_id'];
	$query1 = "Select username from {plugin_Defcon_issue} as a, {user} as b where a.consultant_id=b.id and issue_id=$t_bug_id";
		...
		$variables['consultant'] = $t_consultant;
}
