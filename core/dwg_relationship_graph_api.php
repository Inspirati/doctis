 ...
 * @return Graph
 */
function dwg_relgraph_generate_rel_graph( $p_bug_id, $p_show_summary = false ) {
	...
	while( !empty( $v_queue ) ) {
		...
		$t_bug = dwg_get( $t_id, false );

		if( !access_has_dwg_level( config_get( 'view_dwg_threshold', null, null, $t_bug->project_id ), $t_id ) ) {
			continue;
		}
 ...
 * @return Digraph
 */
function dwg_relgraph_generate_dep_graph( $p_bug_id, $p_horizontal = false, $p_show_summary = false ) {
	# List of visited issues and their data.
	$v_bug_list = array();
	...
	$t_relationships = dwg_relationship_get_all_dest( $p_bug_id );
	foreach( $t_relationships as $t_relationship ) {
 ...
 * @return boolean
 */
function dwg_relgraph_add_parent( array &$p_bug_list, $p_bug_id ) {
	...
	$t_bug = dwg_get( $p_bug_id, false );

	if( !access_has_dwg_level( config_get( 'view_dwg_threshold', null, null, $t_bug->project_id ), $p_bug_id ) ) {
		return false;
	}
 ...
 * @return boolean
 */
function dwg_relgraph_add_child( array &$p_bug_list, $p_bug_id ) {
	...
	if( isset( $p_bug_list[$p_bug_id] ) ) {
		...
		if( !$p_bug_list[$p_bug_id]->is_descendant ) {
			...
			foreach( $p_bug_list[$p_bug_id]->children as $t_child ) {
				dwg_relgraph_add_child( $p_bug_list, $t_child );
			}
		}
	} else {
...
/**
 * Outputs a image for the given relationship graph, previously
 * generated by dwg_relgraph_generate_graph_for_bug().
 * @param Graph $p_graph Graph object.
 * @return void
 */
function dwg_relgraph_output_image( Graph $p_graph ) {
 ...
 * relationship graph.
 * @param Graph  $p_graph Relationship graph object generated from dwg_relgraph_generate_graph_for_bug().
 * @param string $p_name  The XHTML name attribute to apply to the containing <map> element.
 * @return void
 */
function dwg_relgraph_output_map( Graph $p_graph, $p_name ) {
