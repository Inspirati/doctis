 ...
 * @subpackage classes
 */
class DwgStatusChangeTimelineEvent extends TimelineEvent {
	private $issue_id;
	private $old_status;
	private $new_status;
	private $type;
	...
	private function change_type() {
		...
		if( $this->old_status < $t_closed && $this->new_status >= $t_closed ) {
			return DwgStatusChangeTimelineEvent::CLOSED;
		} else if( $this->old_status < $t_resolved && $this->new_status >= $t_resolved ) {
			return DwgStatusChangeTimelineEvent::RESOLVED;
		} else if( $this->old_status >= $t_resolved && $this->new_status < $t_resolved ) {
			return DwgStatusChangeTimelineEvent::REOPENED;
		} else {
			return DwgStatusChangeTimelineEvent::IGNORED;
		}
	}
	...
	public function skip() {
		return $this->type == DwgStatusChangeTimelineEvent::IGNORED;
	}
	...
	public function html() {
		switch( $this->type ) {
			case DwgStatusChangeTimelineEvent::RESOLVED:
                $t_html = $this->html_start( 'fa-thumbs-o-up' );
				$t_string = sprintf(
					lang_get( 'timeline_document_resolved' ),
					prepare_user_name( $this->user_id ),
					string_get_dwg_view_link( $this->issue_id )
				);
				break;
			case DwgStatusChangeTimelineEvent::CLOSED:
                $t_html = $this->html_start( 'fa-power-off' );
				$t_string = sprintf(
					lang_get( 'timeline_document_closed' ),
					prepare_user_name( $this->user_id ),
					string_get_dwg_view_link( $this->issue_id )
				);
				break;
			case DwgStatusChangeTimelineEvent::REOPENED:
                $t_html = $this->html_start( 'fa-refresh' );
				$t_string = sprintf(
					lang_get( 'timeline_document_reopened' ),
					prepare_user_name( $this->user_id ),
					string_get_dwg_view_link( $this->issue_id )
				);
				break;
			case DwgStatusChangeTimelineEvent::IGNORED:
				return '';
			default:
