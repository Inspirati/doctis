...
N_PROJECTS       = 2    # top-level projects
N_SUB            = 1    # sub-projects per top-level project
N_SUBSUB         = 0    # sub-sub-projects per sub-project (set 0 to omit)
N_DOCS_PER_PRJ   = 3    # documents at EACH project level (top + sub + subsub)
N_ISSUES_PER_DOC = 2    # bug/issue rows per document
N_USERS          = 10   # stress users to add (above the existing sample accounts)
N_LICENSES       = 5   # stress licenses to add (above the existing sample licenses)
N_NEWS           = 10   # news stories spread across top-level projects
