 ...
 */
class DwgRelationshipAddCommand extends Command {
	/**
	...
	function validate() {
		...
		$this->sourceIssue = dwg_get( $t_source_issue_id, true );

		$t_update_threshold = config_get( 'update_dwg_threshold', null, null, $this->sourceIssue->project_id );

		# Ensure user has access to update the source issue
		if( !access_has_dwg_level( $t_update_threshold, $t_source_issue_id ) ) {
			throw new ClientException(
		...
		$this->targetIssue = dwg_get( $t_target_issue_id, true );

		# Ensure source issue is not read-only
		if( dwg_is_readonly( $t_source_issue_id ) ) {
			throw new ClientException(
				sprintf( "Document %d is read-only", $t_source_issue_id ),
	...
	protected function process() {
		...
		$t_relationship_id = dwg_relationship_upsert( $this->sourceIssue->id, $this->targetIssue->id, $this->typeId );
	...
	private function getRelationTypeId( $p_relationship_type ) {
		if( isset( $p_relationship_type['id'] ) ) {
			...
			dwg_relationship_type_ensure_valid( $t_type_id );
		} else if( isset( $p_relationship_type['name'] ) ) {
			$t_type_id = dwg_relationship_get_id_from_api_name( $p_relationship_type['name'] );
		} else {
			throw new ClientException(
