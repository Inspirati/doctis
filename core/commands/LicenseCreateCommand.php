 ...
 */
class LicenseCreateCommand extends Command {
	/**
	 * Project Name
	 * @var string
	 */
	...
	function validate() {
		if( !access_has_global_level( config_get( 'create_license_threshold' ) ) ) {
			throw new ClientException(
				'Access denied to create licenses',
				ERROR_ACCESS_DENIED
			);
		}
		...
		$this->view_state = isset( $t_project['view_state'] ) ?  mci_get_license_view_state_id( $t_project['view_state'] ) : config_get( 'default_license_view_status' );
		$this->status = isset( $t_project['status'] ) ? mci_get_license_status_id( $t_project['status'] ) : 10 /* development */;
		$this->enabled = $this->payload( 'enabled', true );

		if( !license_is_name_unique( $this->name ) ) {
			throw new ClientException(
				'License name is not unique',
				ERROR_LICENSE_NAME_NOT_UNIQUE,
				array( 'name' )
			);
		}

		$t_enum_values = MantisEnum::getValues( config_get( 'license_status_enum_string' ) );
		...
		// 	throw new ClientException(
		// 		'Invalid license inherit global',
		// 		ERROR_INVALID_FIELD_VALUE,
	...
	function process() {
		...
		// $t_manage_project_access_level = config_get( 'manage_license_threshold', null, $t_user_id, $t_project_id );
		...
		// 	license_add_user( $t_license_id, $t_user_id, $t_access_level );
		// // }

		// global $g_project_override;
		// $g_project_override = $t_project_id;

		event_signal( 'EVENT_MANAGE_LICENSE_CREATE', array( $t_license_id ) );

		$t_result = array();
		if( $this->option('return_license', false ) ) {
			$t_lang = mci_get_user_lang( $t_user_id );
			$t_result['license'] = mci_license_get( $t_license_id, $t_lang, /* detail */ true );
		} else {
			$t_result['id'] = $t_license_id;
		}
