...
function bug_group_action_get_commands( array $p_project_ids = [] ) {
	...
	foreach( $p_project_ids as $t_project_id ) {
		...
		if( !isset( $t_commands['CLOSE'] ) && $t_update_bug_status_allowed &&
			( access_has_project_level( access_get_bug_status_threshold( config_get( 'bug_closed_status_threshold', null, $t_user_id, $t_project_id ), $t_project_id ), $t_project_id ) ||
		...
		if( !isset( $t_commands['RESOLVE'] ) && $t_update_bug_status_allowed &&
			access_has_project_level( access_get_bug_status_threshold( config_get( 'bug_resolved_status_threshold', null, $t_user_id, $t_project_id ), $t_project_id ), $t_project_id ) ) {
