...
if( $f_bug_id ) {
	...
	$t_bug_revisions = dwg_revision_like( $f_rev_id );
...
if( $t_bugnote_id ) {
	if( !access_can_view_dwgnote_revisions( $t_bugnote_id ) ) {
		access_denied();
	}
} elseif( $t_bug_id && !access_can_view_dwg_revisions( $t_bug_id ) ) {
	access_denied();
}
 ...
 */
function show_dwg_revision( array $p_revision ) {
	...
	switch( $v_type ) {
		...
		case REV_ADDITIONAL_INFO:
			...
			break;
		case REV_DWGNOTE:
			if( !access_has_dwgnote_level( $s_view_bug_threshold, $v_bugnote_id ) ) {
				return;
			}

			$t_label = lang_get( 'dwgnote' );
			break;
		default:
			$t_label = '';
	}
...
<?php
	...
	if( $s_can_drop ) {
		$t_drop_token = form_security_param( 'dwg_revision_drop' );
		print_dwg_link_button(
			'dwg_revision_drop.php?id=' . $v_id . $t_drop_token,
