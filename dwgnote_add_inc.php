...
if( !defined( 'DWGNOTE_ADD_INC_ALLOW' ) ) {
	return;
}
...
?>
<?php if( ( !dwg_is_readonly( $f_bug_id ) ) &&
		( access_has_dwg_level( config_get( 'add_dwgnote_threshold' ), $f_bug_id ) ) ) { ?>
...
<form id="dwgnoteadd"
	method="post"
	action="dwgnote_add.php"
	enctype="multipart/form-data"
	>
	<?php echo form_security_field( 'dwgnote_add' ) ?>
	<input type="hidden" name="bug_id" value="<?php echo $f_bug_id ?>" />
	<div id="dwgnote_add" class="widget-box widget-color-blue2 <?php echo $t_block_css ?>">
...
<?php
	if( config_get( 'time_tracking_enabled' ) ) {
		if( access_has_dwg_level( config_get( 'time_tracking_edit_threshold' ), $f_bug_id ) ) {
?>
			<tr>
				<th class="category">
