 ...
 * @access public
 */
function last_visited_dwg_enabled() {
	return !( 0 == config_get( 'recently_visited_dwg_count' ) || current_user_is_anonymous() );
}
 ...
 * @return void
 */
function last_visited_dwg_issue( $p_issue_id, $p_user_id = null ) {
	if( !last_visited_dwg_enabled() ) {
		return;
	}

	$t_value = token_get_value( TOKEN_LAST_VISITED_DWG, $p_user_id );
	if( is_null( $t_value ) ) {
		$t_value = $p_issue_id;
	} else {
		$t_ids = explode( ',', $p_issue_id . ',' . $t_value );
		$t_ids = array_unique( $t_ids );
		$t_ids = array_slice( $t_ids, 0, config_get( 'recently_visited_dwg_count' ) );
		$t_value = implode( ',', $t_ids );
	}

	token_set( TOKEN_LAST_VISITED_DWG, $t_value, TOKEN_EXPIRY_LAST_VISITED_DWG, $p_user_id );
}
 ...
 * @access public
 */
function last_visited_dwg_get_array( $p_user_id = null ) {
	if( !last_visited_dwg_enabled() ) {
		return array();
	}
