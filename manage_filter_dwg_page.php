...
$t_filter_ids_available =
		filter_dwg_db_get_named_filters( ALL_PROJECTS, $t_user_id, false ) +
		filter_dwg_db_get_named_filters( ALL_PROJECTS, null, true ) +
...
function table_print_filter_row( $p_filter_id ) {
	global $t_rss_enabled;
	$t_editable = filter_dwg_db_can_delete_filter( $p_filter_id );
	echo '<tr>';
	# Filter name
	echo '<td>';
	...
	# Project
	echo '<td>' . string_display_line( project_get_name( filter_dwg_get_field( $p_filter_id, 'project_id' ) ) ) . '</td>';
	# Public
	echo '<td class="center">' . trans_bool( filter_dwg_get_field( $p_filter_id, 'is_public' ) ) . '</td>';
	# Owner
	echo '<td>' . user_get_name( filter_dwg_get_field( $p_filter_id, 'user_id' ) ) . '</td>';
	# Actions
	echo '<td>';
	echo '<div class="pull-left">';
	print_dwg_form_button( 'view_dwg_set.php', lang_get( 'apply_filter_button' ), array( 'type' => 3, 'source_query_id' =>  $p_filter_id ), /* security token */ OFF );
	echo '</div>';
	if( $t_editable ) {
		echo '<div class="pull-left">';
		print_dwg_form_button( 'manage_filter_dwg_edit_page.php', lang_get( 'edit' ), array( 'filter_id' =>  $p_filter_id ) );
		echo '</div>';
		echo '<div class="pull-left">';
		print_dwg_form_button( 'manage_filter_dwg_delete.php', lang_get( 'delete' ), array( 'filter_id' =>  $p_filter_id ) );
		echo '</div>';
	}
	echo '</div>';
	echo '</td>';
