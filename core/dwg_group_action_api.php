 ...
 * @return void
 */
function dwg_group_action_init( $p_action ) {
	$t_valid_actions = dwg_group_action_get_commands( current_user_get_accessible_projects() );
	...
	$t_include_file = config_get_global( 'absolute_path' ) . 'dwg_actiongroup_' . $p_action . '_inc.php';
 ...
 * @return void
 */
function dwg_group_action_print_bottom() {
	layout_page_end();
}
...
function dwg_group_action_print_bug_list( array $p_bug_ids_array ) {
	...
	foreach( $p_bug_ids_array as $t_bug_id ) {
		...
		$t_status_css = html_get_status_css_fg( dwg_get_field( $t_bug_id, 'status' ), auth_get_current_user_id(), dwg_get_field( $t_bug_id, 'project_id' ) );
		$t_lead = icon_get( 'fa-square', 'fa-status-box ' . $t_status_css );
		$t_lead .= ' ' . string_get_dwg_view_link( $t_bug_id );
		echo sprintf( "<tr> <td>%s</td> <td>%s</td> </tr>\n", $t_lead, string_attribute( dwg_get_field( $t_bug_id, 'summary' ) ) );
	}
}
 ...
 * @return void
 */
function dwg_group_action_print_results( array $p_failed_ids ) {
	$t_format = "<tr>"
	...
	foreach( $p_failed_ids as $t_id => $t_reason ) {
		printf( $t_format,
			sprintf( $t_label, string_get_dwg_view_link( $t_id ) ),
			string_display_line( dwg_get_field( $t_id, 'summary' ) ),
			$t_reason
		);
	}
	echo '</table>', PHP_EOL;
	...
	print_link_button( 'view_dwg_page.php', lang_get( 'proceed' ) );
	echo '</div>', PHP_EOL;
}
 ...
 * @return array
 */
function dwg_group_action_get_commands( array $p_project_ids = [] ) {
