 ...
 * @return void
 */
function history_dwg_log_event_direct( $p_bug_id, $p_field_name, $p_old_value, $p_new_value, $p_user_id = null, $p_type = 0 ) {
 ...
 * @return void
 */
function history_dwg_log_event_special( $p_bug_id, $p_type, $p_old_value = '', $p_new_value = '' ) {
	...
	db_param_push();
	$t_query = 'INSERT INTO {dwg_history}
					( user_id, dwg_id, date_modified, type, old_value, new_value, field_name )
				VALUES
					( ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() . ',' . db_param() . ', ' . db_param() . ')';
 ...
 * @return array
 */
function history_dwg_get_events_array( $p_bug_id, $p_user_id = null ) {
	$t_normal_date_format = config_get( 'normal_date_format' );

	$t_raw_history = history_dwg_get_raw_events_array( $p_bug_id, $p_user_id );
	$t_history = array();
	...
	foreach( $t_raw_history as $k => $t_item ) {
		...
		$t_history[$k] = history_dwg_localize_item( $p_bug_id, $v_field, $v_type, $v_old_value, $v_new_value );
 ...
 */
function history_dwg_count_user_recent_events( $p_duration_in_seconds, $p_user_id = null ) {
	$t_user_id = ( ( null === $p_user_id ) ? auth_get_current_user_id() : $p_user_id );

	$t_params = array( db_now() - $p_duration_in_seconds, $t_user_id );

	db_param_push();
	$t_query = 'SELECT count(*) as event_count FROM {dwg_history} WHERE date_modified > ' . db_param() .
 ...
 */
function history_dwg_query_result( array $p_query_options ) {
	...
	if( isset( $p_query_options['start_time'] ) ) {
		$t_where[] = '{dwg_history}.date_modified >= ' . $t_query->param( (int)$p_query_options['start_time'] );
	}

	# End time
	if( isset( $p_query_options['end_time'] ) ) {
		$t_where[] = '{dwg_history}.date_modified < ' . $t_query->param( (int)$p_query_options['end_time'] );
	}

	# Bug ids
	if( isset( $p_query_options['dwg_id'] ) ) {
		$c_ids = array();
		if( is_array( $p_query_options['dwg_id'] ) ) {
			foreach( $p_query_options['dwg_id'] as $t_id ) {
				$c_ids[] = (int)$t_id;
			}
		} else {
			$c_ids[] = (int)$p_query_options['dwg_id'];
		}
		$t_where[] = $t_query->sql_in( '{dwg_history}.dwg_id', $c_ids );
	}
	...
	if( isset( $p_query_options['user_id'] ) ) {
		...
		$t_where[] = $t_query->sql_in( '{dwg_history}.user_id', $c_ids );
	}

	$t_query->append_sql( 'SELECT * FROM {dwg_history}' );
	if ( count( $t_where ) > 0 ) {
	...
	$t_query->append_sql( ' ORDER BY {dwg_history}.date_modified ' . $t_history_order . ', {dwg_history}.id ' . $t_history_order );
...
function history_dwg_get_range_result_filter( $p_filter, $p_start_time = null, $p_end_time = null, $p_history_order = null ) {
	...
	return history_dwg_query_result( $t_query_options );
}
...
function history_dwg_get_range_result( $p_bug_id = null, $p_start_time = null, $p_end_time = null, $p_history_order = null ) {
	...
	if ( $p_bug_id !== null ) {
		$t_query_options['dwg_id'] = $p_bug_id;
	}
	return history_dwg_query_result( $t_query_options );
}
 ...
 */
function history_dwg_get_event_from_row( $p_result, $p_user_id = null, $p_check_access_to_issue = true ) {
	...
	while ( $t_row = db_fetch_array( $p_result ) ) {
		...
		if( $t_user_id != $v_user_id ) {
			...
			if( ( $v_type == DWGNOTE_ADDED ) || ( $v_type == DWGNOTE_UPDATED ) || ( $v_type == DWGNOTE_DELETED ) ) {
				if( !dwgnote_exists( $v_old_value ) ) {
					continue;
				}
			...
			if( $v_type == DWGNOTE_STATE_CHANGED ) {
				if( !dwgnote_exists( $v_new_value ) ) {
					continue;
				}
		...
		if( $v_type == BUG_ADD_RELATIONSHIP || $v_type == BUG_DEL_RELATIONSHIP || $v_type == BUG_REPLACE_RELATIONSHIP ) {
			...
			if( !dwg_exists( $t_related_bug_id ) || !access_has_dwg_level( config_get( 'view_dwg_threshold' ), $t_related_bug_id, $t_user_id ) ) {
				continue;
			}
		}
 ...
 * @return array
 */
function history_dwg_get_raw_events_array( $p_bug_id, $p_user_id = null, $p_start_time = null, $p_end_time = null ) {
	$t_user_id = (( null === $p_user_id ) ? auth_get_current_user_id() : $p_user_id );
	...
	$t_result = history_dwg_query_result( $t_query_options );

	$t_raw_history = array();

	$j = 0;
	while( true ) {
		$t_event = history_dwg_get_event_from_row( $t_result, $t_user_id, /* check access */ true );
		if ( $t_event === false ) {
			break;
		}
 ...
 */
function history_dwg_localize_field_name( $p_field_name ) {
	switch( $p_field_name ) {
		case 'category':
 ...
 */
function history_dwg_get_type_name( $p_type ) {
	$t_type = (int)$p_type;
 ...
 */
function history_dwg_localize_item( $p_bug_id, $p_field_name, $p_type, $p_old_value, $p_new_value, $p_linkify = true ) {
	$t_note = '';
	$t_change = '';
	$t_raw = true;
	...
	if( NORMAL_TYPE != $p_type ) {
		switch( $p_type ) {
			...
			case DWGNOTE_ADDED:
				...
				break;
			case DWGNOTE_UPDATED:
				$t_note = lang_get( 'dwgnote_edited' ) . ': ' . $p_old_value;
				$t_old_value = (int)$p_old_value;
				$t_new_value = (int)$p_new_value;
				if( $p_linkify
					&& dwg_revision_exists( $t_new_value )
					&& access_can_view_dwgnote_revisions( $t_old_value )
				) {
					if( dwgnote_exists( $t_old_value ) ) {
						$t_bug_revision_view_page_argument = 'dwgnote_id=' . $t_old_value . '#r' . $t_new_value;
					} else {
						$t_bug_revision_view_page_argument = 'rev_id=' . $t_new_value;
					}
					$t_change = '<a href="dwg_revision_view_page.php?' . $t_bug_revision_view_page_argument . '">' .
						lang_get( 'view_revisions' ) . '</a>';
					$t_raw = false;
				}
				break;
			case DWGNOTE_DELETED:
				$t_note = lang_get( 'dwgnote_deleted' ) . ': ' . $p_old_value;
				break;
			case DESCRIPTION_UPDATED:
			case STEP_TO_REPRODUCE_UPDATED:
			case ADDITIONAL_INFO_UPDATED:
				...
				$t_old_value = (int)$p_old_value;
				if( $p_linkify
					&& dwg_revision_exists( $t_old_value )
					&& access_can_view_bug_revisions( $p_bug_id )
				) {
					$t_change = '<a href="dwg_revision_view_page.php?rev_id=' . $t_old_value . '#r' . $t_old_value . '">' .
						lang_get( 'view_revisions' ) . '</a>';
					$t_raw = false;
				}
				break;
			...
			case DWG_CREATED_FROM:
				...
				break;
			case TAG_ATTACHED:
				$t_note = lang_get( 'tag_history_dwg_attached' ) . ': ' . $p_old_value;
				break;
			case TAG_DETACHED:
				$t_note = lang_get( 'tag_history_dwg_detached' ) . ': ' . $p_old_value;
				break;
			case TAG_RENAMED:
			...
			case DWG_REVISION_DROPPED:
				...
				break;
			case DWGNOTE_REVISION_DROPPED:
				$t_note = lang_get( 'dwgnote_revision_dropped_history' ) . ': '
					. $p_new_value . ': '
					. $p_old_value;
				break;
		}
 ...
 * @return void
 */
function history_dwg_delete( $p_bug_id ) {
	db_param_push();
	$t_query = 'DELETE FROM {dwg_history} WHERE dwg_id=' . db_param();
 ...
 * @return void
 */
function history_dwg_link_file_to_bugnote( $p_bug_id, $p_filename, $p_bugnote_id ) {
	db_param_push();
	$t_query = 'UPDATE {dwg_history} SET new_value = ' . db_param() .
		' WHERE dwg_id=' . db_param() . ' AND old_value=' . db_param() .
