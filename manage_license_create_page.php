...
access_ensure_global_level( config_get( 'create_license_threshold' ) );
...
<?php } ?>
	...
	<form method="post" id="manage-project-create-form" action="manage_license_create.php">
...
<?php
	...
	if( null !== $f_parent_id ) {
		...
		echo lang_get( 'add_license_title' );
	}
?>
			</h4>
		</div>
		...
		<fieldset>
<?php
	echo form_security_field( 'manage_license_create' );
	if( null !== $f_parent_id ) {
			...
			<tr>
				...
				<td>
					<select id="project-status" name="status" class="input-sm">
						<?php print_enum_string_option_list( 'license_status' ) ?>
					</select>
				</td>
			</tr>
<?php /*
...
<?php
	print_enum_string_option_list(
		'license_view_state',
		config_get( 'default_license_view_status',
				null,
				ALL_USERS,
				ALL_PROJECTS )
		);
...
*/ ?>
			...
			<?php event_signal( 'EVENT_MANAGE_LICENSE_CREATE_FORM' ) ?>
		</table>
		</fieldset>
		</div>
		</div>
		...
		<div class="widget-toolbox padding-8 clearfix">
			...
			<input type="submit" class="btn btn-primary btn-white btn-round"
				   value="<?php echo lang_get( 'add_license_button' ) ?>" />
		</div>
	</div>
	</form>
	</div>
