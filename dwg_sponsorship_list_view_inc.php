...
if( ( config_get( 'enable_sponsorship' ) == ON ) && ( access_has_dwg_level( config_get( 'view_sponsorship_total_threshold' ), $f_bug_id ) ) ) {
	$t_sponsorship_ids = sponsorship_get_all_ids( $f_bug_id );

	$t_sponsorships_exist = count( $t_sponsorship_ids ) > 0;
	$t_can_sponsor = !dwg_is_readonly( $f_bug_id ) && !current_user_is_anonymous();
...
<table class="width100" cellspacing="1">
	<tr>
		...
		<td class="form-title" colspan="2"><?php
			...
			echo lang_get( 'users_sponsoring_dwg' );
...
?>
	<tr>
		...
		<td>
			<form method="post" action="dwg_set_sponsorship.php">
				<?php echo form_security_field( 'dwg_set_sponsorship' ) ?>
...
<?php
	...
	$t_total_sponsorship = dwg_get_field( $f_bug_id, 'sponsorship_total' );
...
?>
	<tr>
		...
		<?php
			...
			if( access_has_dwg_level( config_get( 'view_sponsorship_details_threshold' ), $f_bug_id ) ) {
				echo '<br /><br />';
				$i = 0;
				foreach ( $t_sponsorship_ids as $t_id ) {
					...
					if( access_has_dwg_level( config_get( 'handle_sponsored_dwgs_threshold' ), $f_bug_id ) ) {
