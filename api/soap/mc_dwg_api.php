 ...
 */
function mc_dwg_exists( $p_username, $p_password, $p_issue_id ) {
 ...
 * @return array that represents an DwgData structure
 */
...
*/
function mc_dwg_get_history( $p_username, $p_password, $p_issue_id ) {
 ...
 */
function mci_dwg_set_custom_fields( $p_issue_id, ?array &$p_custom_fields, $p_log_insert = true ) {
...
function mci_dwg_get_attachments( $p_issue_id, $p_note_id = null ) {
	...
	foreach( $t_attachment_rows as $t_attachment_row ) {
		...
		if( ApiObjectFactory::$soap ) {
			...
			$t_attachment['creator'] = mci_account_get_array_by_id( $t_attachment_row['user_id'] );
...
function mci_dwg_get_relationships( $p_issue_id, $p_user_id ) {
	...
	foreach( $t_src_relationships as $t_relship_row ) {
		if( access_has_dwg_level( config_get( 'webservice_readonly_access_level_threshold' ), $t_relship_row->dest_dwg_id, $p_user_id ) ) {
			$t_related_issue_id = (int)$t_relship_row->dest_dwg_id;
	...
	foreach( $t_dest_relationships as $t_relship_row ) {
		if( access_has_dwg_level( config_get( 'webservice_readonly_access_level_threshold' ), $t_relship_row->src_dwg_id, $p_user_id ) ) {
			$t_relationship = array();
			$t_relationship['id'] = (int)$t_relship_row->id;
			$t_reltype = array();
			...
			$t_relationship['type'] = $t_reltype;
			$t_related_issue_id = (int)$t_relship_row->src_dwg_id;
...
function mci_dwg_note_data_as_array( $p_bugnote_row ) {
	...
	$t_lang = mci_get_user_lang( $t_user_id );
	$t_has_time_tracking_access = access_has_dwg_level( config_get( 'time_tracking_view_threshold' ), $p_bugnote_row->dwg_id );
	...
	if( ApiObjectFactory::$soap ) {
		...
		$t_bugnote['attachments'] = mci_dwg_get_attachments( $p_bugnote_row->dwg_id, $p_bugnote_row->id );
 ...
 * @return array that represents an SOAP DwgNoteData structure
 */
function mci_dwg_get_notes( $p_issue_id ) {
	...
	$t_result = array();
	foreach( dwgnote_get_all_visible_dwgnotes( $p_issue_id, $t_user_bugnote_order, 0 ) as $t_value ) {
 ...
 */
function mc_dwg_get_biggest_id( $p_username, $p_password, $p_project_id ) {
	global $g_project_override;
	...
	$t_rows = filter_dwg_get_dwg_rows( $t_page_number, $t_per_page, $t_page_count, $t_bug_count, $t_filter, $t_project_id, $t_user_id );
	if( count( $t_rows ) == 0 ) {
		return 0;
	} else {
		return $t_rows[0]->id;
 ...
 */
function mci_dwg_handler_access_check( $p_user_id, $p_project_id, $p_old_handler_id, $p_new_handler_id ) {
	if( $p_new_handler_id != 0 ) {
		...
		if( !access_has_project_level( config_get( 'handle_dwg_threshold' ), $p_project_id, $p_new_handler_id ) ) {
			return mci_fault_access_denied( $p_new_handler_id, 'User does not have access right to handle documents' );
		}
	}

	if( $p_old_handler_id != $p_new_handler_id ) {
		if( !access_has_project_level( config_get( 'update_dwg_assign_threshold' ), $p_project_id, $p_user_id ) ) {
			return mci_fault_access_denied( $p_user_id, 'User does not have access right to assign documents' );
		}
	}
 ...
 * @param stdClass $p_issue    A DwgData structure containing information
 *                             about the new issue.
 *
 * @return true|RestFault|SoapFault
 * @throws ClientException
 */
function mc_dwg_update( $p_username, $p_password, $p_issue_id, stdClass $p_issue ) {
	global $g_project_override;
	...
	if( !access_has_dwg_level( config_get( 'update_dwg_threshold' ), $p_issue_id, $t_user_id ) ) {
		return mci_fault_access_denied( $t_user_id, 'Not enough rights to update documents' );
	}
	...
	if( $t_bug_data->handler_id != $t_handler_id ) {
		$t_access_check_result = mci_dwg_handler_access_check(
	...
	if( isset( $p_issue['status'] ) ) {
		...
		if( $t_resolving &&
			!relationship_can_resolve_dwg( $p_issue_id ) &&
			OFF == config_get( 'allow_parent_of_unresolved_to_close' )
		) {
			return ApiObjectFactory::faultBadRequest( 'Unresolved child documents.' );
		}
	...
	if( isset( $p_issue['notes'] ) && is_array( $p_issue['notes'] ) ) {
		$t_bugnotes = dwgnote_get_all_visible_dwgnotes( $p_issue_id, 'DESC', 0 );
			...
			if( isset( $t_note['id'] ) && ( (int)$t_note['id'] > 0 ) ) {
				...
				if( array_key_exists( $t_bugnote_id, $t_bugnotes_by_id ) ) {
					...
					if( isset( $t_note['time_tracking']) && $t_note['time_tracking'] != $t_bugnote->time_tracking ) {
						dwgnote_set_time_tracking( $t_bugnote_id, mci_dwg_get_time_tracking_from_note( $p_issue_id, $t_note ) );
						$t_bugnote_changed = true;
					}
				...
				dwgnote_add( $p_issue_id, $t_note['text'], mci_dwg_get_time_tracking_from_note( $p_issue_id, $t_note ), $t_view_state_id == VS_PRIVATE, $t_note_type, $t_note_attr, $t_user_id, false );
			}
		}
 ...
 * @return mixed
 */
function mc_dwg_set_tags ( $p_username, $p_password, $p_issue_id, array $p_tags ) {
	global $g_project_override;
 ...
 */
function mc_dwg_delete( $p_username, $p_password, $p_issue_id ) {
	$t_user_id = mci_check_login( $p_username, $p_password );
	if( $t_user_id === false ) {
		return mci_fault_login_failed();
	}
 ...
 */
function mc_dwg_note_add( $p_username, $p_password, $p_issue_id, stdClass $p_note ) {
	$t_user_id = mci_check_login( $p_username, $p_password );
	if( $t_user_id === false ) {
		return mci_fault_login_failed();
	}
	...
	if( $t_note_type != REMINDER ) {
		...
		if( isset( $p_note['creator'] ) ) {
			$t_payload['creator'] = array( 'id' => mci_get_user_id( $p_note['creator'] ) );
		}
...
//	creator
	...
	# TODO: #17777: Add test case for mc_issue_add() and mc_issue_note_add() creator override
	if( isset( $p_note['creator'] ) ) {
		...
		if( $t_creator_id != $t_user_id ) {
			...
			if( !access_has_project_level( $t_specify_creator_access_level, $t_project_id, $t_user_id ) ) {
	...
	$t_bugnote_id = dwgnote_add( $p_issue_id, $p_note['text'], mci_dwg_get_time_tracking_from_note( $p_issue_id, $p_note ), $t_view_state_id == VS_PRIVATE, $t_note_type, $t_note_attr, $t_creator_id );
 ...
 */
function mc_dwg_note_delete( $p_username, $p_password, $p_issue_note_id ) {
	$t_user_id = mci_check_login( $p_username, $p_password );
	if( $t_user_id === false ) {
		return mci_fault_login_failed();
	}
 ...
 */
function mc_dwg_note_update( $p_username, $p_password, stdClass $p_note ) {
	global $g_project_override;
	...
	log_event( LOG_WEBSERVICE, 'updating dwgnote id \'' . $t_issue_note_id . '\'' );
 ...
 */
function mc_dwg_relationship_add( $p_username, $p_password, $p_issue_id, stdClass $p_relationship ) {
	global $g_project_override;
	$t_user_id = mci_check_login( $p_username, $p_password );
	...
	if( !dwg_exists( $t_dest_issue_id ) ) {
		return ApiObjectFactory::faultNotFound( 'Document \'' . $t_dest_issue_id . '\' not found.' );
	}
 ...
 * @return boolean true: success, false: failure
 */
function mc_dwg_relationship_delete( $p_username, $p_password, $p_issue_id, $p_relationship_id ) {
	global $g_project_override;
	...
	# user has access to update the bug...
	if( !access_has_dwg_level( config_get( 'update_dwg_threshold' ), $p_issue_id, $t_user_id ) ) {
		return mci_fault_access_denied( $t_user_id, 'Active user does not have access level required to remove a relationship from this document.' );
	}
...
function mci_dwg_data_as_array( DwgData $p_issue_data, $p_user_id, $p_lang, $p_fields = null ) {
	...
	if( $t_fields === null || isset( $t_fields['reference'] ) ) {
		$t_issue['reference'] = mci_sanitize_xml_string( $p_issue_data->reference );
	}
	...
	if( $t_fields === null || isset( $t_fields['release_date'] ) ) {
		$t_issue['release_date'] = mci_sanitize_xml_string( $p_issue_data->release_date );
	}

	// if( $t_fields === null || isset( $t_fields['date_submitted'] ) ) {
	// 	$t_issue['date_submitted'] = mci_sanitize_xml_string( $p_issue_data->date_submitted );
	// }
	// last_updated is already handled below..
	// if( $t_fields === null || isset( $t_fields['last_updated'] ) ) {
	// 	$t_issue['last_updated'] = mci_sanitize_xml_string( $p_issue_data->last_updated );
	// }
	...
	if( $t_fields === null || isset( $t_fields['summary'] ) ) {
	...
	if( $t_fields === null || isset( $t_fields['category'] ) ) {
	...
	if( $t_fields === null || isset( $t_fields['fixed_in_version'] ) ) {
	...
	if( $t_fields === null ||
		isset( $t_fields['platform'] ) ||
		isset( $t_fields['os'] ) ||
		isset( $t_fields['os_build'] )
	) {
		if( config_get( 'allow_freetext_in_profile_fields' ) != OFF ) {
			if( $t_fields === null || isset( $t_fields['platform'] ) ) {
				$t_issue['platform'] = mci_null_if_empty( $p_issue_data->platform );
			}

			if( $t_fields === null || isset( $t_fields['os'] ) ) {
				$t_issue['os'] = mci_null_if_empty( $p_issue_data->os );
			}

			if( $t_fields === null || isset( $t_fields['os_build'] ) ) {
				$t_issue['os_build'] = mci_null_if_empty( $p_issue_data->os_build );
			}
		}
	}

	if( $t_fields === null || isset( $t_fields['eta'] ) ) {
	...
	if( ApiObjectFactory::$soap ) {
		# No need to optimize the SOAP path and fields processed here are cheap anyways
		...
		if( $t_fields === null || isset( $t_fields['sticky'] ) ) {
			$t_issue['sticky'] = (bool)$p_issue_data->sticky;
		}

		if( $t_fields === null || isset( $t_fields['created_at'] ) ) {
			$t_issue['created_at'] = $t_created_at;
		}

		if( $t_fields === null || isset( $t_fields['updated_at'] ) ) {
			$t_issue['updated_at'] = $t_updated_at;
		}
	}

	if( $t_fields === null || isset( $t_fields['attachments'] ) ) {
		# Get attachments - access checked as part of returning attachments
		$t_issue['attachments'] = mci_dwg_get_attachments( $p_issue_data->id, /* note_id */ 0 );
	}

	# Get notes - access checked as part of returning notes.
	if( $t_fields === null || isset( $t_fields['notes'] ) ) {
		$t_issue['notes'] = mci_dwg_get_notes( $p_issue_data->id );
	}

	# Get attachments - access checked as part of returning relationships
	if( $t_fields === null || isset( $t_fields['relationships'] ) ) {
		$t_issue['relationships'] = mci_dwg_get_relationships( $p_issue_data->id, $p_user_id );
	}

	# Get custom fields - access checked as part of returning custom fields
	if( $t_fields === null || isset( $t_fields['custom_fields'] ) ) {
		$t_issue['custom_fields'] = mci_dwg_get_custom_fields( $p_issue_data->id );
	}

	# Get tags - access checked as part of returning tags
	if( $t_fields === null || isset( $t_fields['tags'] ) ) {
		$t_issue['tags'] = mci_dwg_get_tags_for_bug_id( $p_issue_data->id, $p_user_id );
	}
	...
	if( !ApiObjectFactory::$soap ) {
		if( $t_fields === null || isset( $t_fields['history'] ) ) {
			$t_history = mci_dwg_get_history( $p_issue_data->id, $p_user_id, $p_lang );
			if( !empty( $t_history ) ) {
...
function mci_dwg_data_as_header_array( DwgData $p_issue_data ) {
		...
		$t_attachments = mci_dwg_get_attachments( $p_issue_data->id );
		$t_issue['attachments_count'] = $t_attachments === null ? 0 : count( $t_attachments );

		$t_notes = mci_dwg_get_notes( $p_issue_data->id );
...
function mc_dwgs_get( $p_username, $p_password, $p_issue_ids ) {
	...
	foreach( $p_issue_ids as $t_id ) {
		...
		$t_result[] = mci_dwg_data_as_array( $t_issue_data, $t_user_id, $t_lang );
	}
 ...
 */
function mc_dwg_hash( $p_issue_id, $p_issue, $p_user_id = null ) {
	if( $p_user_id === null ) {
