...
form_security_validate( 'manage_license_user_update' );
auth_reauthenticate();
...
access_ensure_license_level( config_get( 'license_user_threshold' ), $f_license_id );
...
$t_all_local_users = license_get_all_user_rows( $f_license_id, ANYBODY, false );
...
foreach( $f_form_deleted_ids as $t_id ) {
	...
	$t_can_manage_this_user = access_has_license_level( $t_user['access_level'], $f_license_id );
...
$t_current_user_access_level = access_get_license_level( $f_license_id );
...
if( !$f_confirmed ) {
		...
		<div class="widget-box widget-color-grey">
			<div class="widget-header widget-header-small">
				<h4 class="widget-title lighter">
					...
					<?php echo lang_get( 'remove_license_user_title' ) ?>
				</h4>
			</div>
			<div class="widget-body">
				<div class="widget-main no-padding">
		...
		<div class="widget-box widget-color-grey">
			<div class="widget-header widget-header-small">
				<h4 class="widget-title lighter">
					...
					<?php echo lang_get( 'modify_license_user_title' ) ?>
				</h4>
			</div>
			<div class="widget-body">
				<div class="widget-main no-padding">
	...
	if( !empty( $t_users_to_update ) ) {
		// license_add_users( $f_license_id, $f_license_id, $t_users_to_update );
		license_add_users( $f_license_id, $t_users_to_update );
	}
	if( !empty( $t_users_to_delete ) ) {
		license_remove_users( $f_license_id, $t_users_to_delete );
	}

	form_security_purge( 'manage_license_user_update' );
	
	// $t_redirect_url = 'manage_license_edit_page.php?project_id=' . $f_license_id . '#license-users';
