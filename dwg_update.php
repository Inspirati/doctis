 ...
 *
 * @param DwgData $p_bug  Reference document
 * @param string $p_field Field name (used for GPC var and DwgData property)
 *
 * @return string|null
 */
function get_valid_version( DwgData $p_bug, $p_field ) {
...
form_security_validate( 'dwg_update' );

$f_bug_id = gpc_get_int( 'bug_id' );
$t_existing_bug = dwg_get( $f_bug_id, true );
$f_update_type = gpc_get_string( 'action_type', DWG_UPDATE_TYPE_NORMAL );
...
if( $t_existing_bug->last_updated != $t_updated_bug->last_updated ) {
	trigger_error( ERROR_DWG_CONFLICTING_EDIT, ERROR );
}
...
$t_closed_status = config_get( 'dwg_closed_status_threshold' );
$t_reopen_resolution = config_get( 'dwg_reopen_resolution' );
...
if( $t_existing_bug->status < $t_resolved_status &&
	...
	$t_close_issue = true;
		   ...
		   $t_updated_bug->status <= config_get( 'dwg_reopen_status' )
) {
	$t_reopen_issue = true;
}
...
if ( !$t_creator_reopening && !$t_creator_closing ) {
	switch( $f_update_type ) {
		case DWG_UPDATE_TYPE_ASSIGN:
			...
			break;
		case DWG_UPDATE_TYPE_CLOSE:
		case DWG_UPDATE_TYPE_REOPEN:
			$t_threshold = 'update_dwg_status_threshold';
			$t_check_readonly = false;
			break;
		case DWG_UPDATE_TYPE_CHANGE_STATUS:
			$t_threshold = 'update_dwg_status_threshold';
			$t_check_readonly = true;
			break;
		case DWG_UPDATE_TYPE_NORMAL:
		default:
			$t_threshold = 'update_dwg_threshold';
			$t_check_readonly = true;
			break;
	}
	access_ensure_dwg_level( config_get( $t_threshold ), $f_bug_id );
...
if( ( $t_resolve_issue || $t_close_issue ) &&
	!dwg_relationship_can_resolve_dwg( $f_bug_id ) &&
	OFF == config_get( 'allow_parent_of_unresolved_to_close' ) ) {
	trigger_error( ERROR_DWG_RESOLVE_DEPENDANTS_BLOCKING, ERROR );
}

# Validate any change to the status of the issue.
if( $t_existing_bug->status != $t_updated_bug->status ) {
	if( !dwg_check_workflow( $t_existing_bug->status, $t_updated_bug->status ) ) {
...
if( $t_existing_bug->handler_id != $t_updated_bug->handler_id ) {
	...
	access_ensure_dwg_level( config_get( 'update_dwg_assign_threshold' ), $f_bug_id );
	if( $t_issue_is_sponsored && !access_has_project_level( config_get( 'handle_sponsored_dwgs_threshold' ),  $t_updated_bug->project_id, $t_updated_bug->handler_id ) ) {
		trigger_error( ERROR_SPONSORSHIP_HANDLER_ACCESS_LEVEL_TOO_LOW, ERROR );
	}
	if( $t_updated_bug->handler_id != NO_USER ) {
		if( !access_has_project_level( config_get( 'handle_dwg_threshold' ),  $t_updated_bug->project_id, $t_updated_bug->handler_id ) ) {
			trigger_error( ERROR_HANDLER_ACCESS_TOO_LOW, ERROR );
		}
		if( $t_issue_is_sponsored && !access_has_dwg_level( config_get( 'assign_sponsored_dwgs_threshold' ), $f_bug_id ) ) {
...
if( $t_existing_bug->target_version !== $t_updated_bug->target_version ) {
	access_ensure_dwg_level( config_get( 'roadmap_update_threshold' ), $f_bug_id );
}
...
if( $t_existing_bug->view_state != $t_updated_bug->view_state ) {
	access_ensure_dwg_level( config_get( 'change_view_status_threshold' ), $f_bug_id );
}
...
if( $t_updated_bug->duplicate_id != 0 ) {
	if( $t_updated_bug->duplicate_id == $f_bug_id ) {
		trigger_error( ERROR_DWG_DUPLICATE_SELF, ERROR );
	}
...
if( $t_bug_note->note &&
	...
	if( $t_updated_bug->handler_id != NO_USER ) {
		...
		$t_updated_bug->status = config_get( 'dwg_submit_status' );
	}
}

# Handle automatic assignment of issues.
$t_updated_bug->status = dwg_get_status_for_assign( $t_existing_bug->handler_id, $t_updated_bug->handler_id, $t_existing_bug->status, $t_updated_bug->status );
...
helper_call_custom_function( 'document_update_validate', array( $f_bug_id, $t_updated_bug, $t_bug_note->note ) );

# Allow plugins to validate/modify the update prior to it being committed.
$t_updated_bug = event_signal( 'EVENT_UPDATE_DWG_DATA', $t_updated_bug, $t_existing_bug );
...
if( $t_bug_note->note || helper_duration_to_minutes( $t_bug_note->time_tracking ) > 0 ) {
	$t_bugnote_id = dwgnote_add( $f_bug_id, $t_bug_note->note, $t_bug_note->time_tracking, $t_bug_note->view_state == VS_PRIVATE, 0, '', null, false );
	dwgnote_process_mentions( $f_bug_id, $t_bugnote_id, $t_bug_note->note );
}
...
if( $t_updated_bug->duplicate_id != 0 ) {
	...
	dwg_monitor_copy( $f_bug_id, $t_updated_bug->duplicate_id );
}

event_signal( 'EVENT_UPDATE_DWG', array( $t_existing_bug, $t_updated_bug ) );
...
# the new plugin system instead.
helper_call_custom_function( 'document_update_notify', array( $f_bug_id ) );

# Send a notification of changes via email.
if( $t_resolve_issue ) {
	email_dwg_resolved( $f_bug_id );
	email_dwg_relationship_child_resolved( $f_bug_id );
} else if( $t_close_issue ) {
	email_dwg_close( $f_bug_id );
	email_dwg_relationship_child_closed( $f_bug_id );
} else if( $t_reopen_issue ) {
	email_dwg_reopened( $f_bug_id );
	...
	email_dwg_updated( $f_bug_id );
}

form_security_purge( 'dwg_update' );
