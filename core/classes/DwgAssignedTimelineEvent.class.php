 ...
 * @subpackage classes
 */
class DwgAssignedTimelineEvent extends TimelineEvent {
	private $issue_id;
	private $handler_id;
	...
	public function html() {
		if( $this->user_id == $this->handler_id ) {
			...
			$t_string = sprintf( lang_get( 'timeline_document_assigned_to_self' ), prepare_user_name( $this->user_id ), string_get_dwg_view_link( $this->issue_id ) );
		} else if( $this->handler_id != NO_USER ) {
			$t_html = $this->html_start( 'fa-hand-o-right' );
			$t_string = sprintf( lang_get( 'timeline_document_assigned' ), prepare_user_name( $this->user_id ), string_get_dwg_view_link( $this->issue_id ), prepare_user_name( $this->handler_id ) );
		} else {
            $t_html = $this->html_start( 'fa-flag-o' );
			$t_string = sprintf( lang_get( 'timeline_document_unassigned' ), prepare_user_name( $this->user_id ), string_get_dwg_view_link( $this->issue_id ) );
		}
