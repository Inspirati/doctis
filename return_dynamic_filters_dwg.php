...
# call to filter_dwg_ensure_valid_filter to clean up after adding unsafe values from gpc vars
 ...
 * @return void
 */
function return_dynamic_filters_dwg_prepend_headers() {
	if( !headers_sent() ) {
...
$t_found = false;
try {
	$t_content = filter_dwg_form_get_input( $t_filter, $filter_target );
} catch( StateException $e ) {
	$t_content = false;
}

if( false !== $t_content ) {
	return_dynamic_filters_dwg_prepend_headers();
	$t_found = true;
	echo $t_content;
	...
	if( $t_cfdef && access_has_any_project_level( $t_cfdef['access_level_r'] ) && $t_cfdef['filter_by'] ) {
		$t_found = true;
		return_dynamic_filters_dwg_prepend_headers();
		print_filter_dwg_custom_field( $t_custom_id, $t_filter );
	} else {
		trigger_error( ERROR_ACCESS_DENIED, ERROR );
	}
} else {
