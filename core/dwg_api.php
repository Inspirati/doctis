...
//		if( 0 == $this->category_id && !config_get( 'allow_no_document' ) ) {  // @TODO RobD:
	...
	public function create() {
		...
		$this->steps_to_reproduce = isset($this->steps_to_reproduce) ? $this->steps_to_reproduce : '';
		$this->additional_information = isset($this->additional_information) ? $this->additional_information : '';
 ...
 * @access public
 */
function dwg_get_newest_dwgnote_timestamp( $p_bug_id ) {
	$c_bug_id = (int)$p_bug_id;
 ...
 * @access public
 */
function dwg_get_dwgnote_stats_array( array $p_bugs_id, $p_user_id = null ) {
	if( empty( $p_bugs_id ) ) {
		return array();
	}
		...
		while( $t_query_row = $t_query->fetch() ) {
			...
			if( 0 == $t_counter || $t_current_project_id !== $t_query_row['project_id'] ) {
				...
				$t_private_bugnote_visible = access_compare_level(
					$t_user_access_level,
					config_get( 'private_dwgnote_threshold', null, $t_user_id, $t_query_row['project_id'] )
				);
			}
...
function dwg_get_bugnote_stats( $p_bug_id ) {
	...
	if( array_key_exists( '_stats', $g_cache_dwg[$c_bug_id] ) ) {
		...
		$t_stats = dwg_get_dwgnote_stats_array( array( $p_bug_id ) );
...
function dwg_assign( $p_bug_id, $p_user_id, $p_bugnote_text = '', $p_bugnote_private = false ) {
	if( $p_user_id != NO_USER ) {
		...
		if( !access_has_project_level( config_get( 'handle_dwg_threshold' ), $t_project_id, $p_user_id ) ) {
			trigger_error( ERROR_HANDLER_ACCESS_TOO_LOW, ERROR );
		}
		if( $t_bug_sponsored && !access_has_project_level( config_get( 'handle_sponsored_dwgs_threshold' ), $t_project_id, $p_user_id ) ) {
	...
	if( ( $t_ass_val != $h_status ) || ( $p_user_id != $h_handler_id ) ) {
		...
		if( !is_blank( $p_bugnote_text ) ) {
			$t_bugnote_id = dwgnote_add( $p_bug_id, $p_bugnote_text, 0, $p_bugnote_private, 0, '', null, false );
			dwgnote_process_mentions( $p_bug_id, $t_bugnote_id, $p_bugnote_text );
		}
...
function dwg_close( $p_bug_id, $p_bugnote_text = '', $p_bugnote_private = false, $p_time_tracking = '0:00' ) {
	...
	# Moved dwgnote_add before dwg_set_field calls in case time_tracking_no_note is off.
	# Error condition stopped execution but status had already been changed
	if( !is_blank( $p_bugnote_text ) || $p_time_tracking != '0:00' ) {
		$t_bugnote_id = dwgnote_add( $p_bug_id, $p_bugnote_text, $p_time_tracking, $p_bugnote_private, 0, '', null, false );
		dwgnote_process_mentions( $p_bug_id, $t_bugnote_id, $p_bugnote_text );
	}
...
function dwg_monitor( $p_bug_id, $p_user_id ) {
	...
	db_param_push();
	$t_query = 'INSERT INTO {dwg_monitor} ( user_id, dwg_id ) VALUES (' . db_param() . ',' . db_param() . ')';
	...
	email_dwg_monitor_added( $p_bug_id, $p_user_id );
...
function dwg_get_monitors( $p_bug_id ) {
	...
	$t_query = 'SELECT user_id, enabled
			...
			WHERE m.dwg_id=' . db_param() . ' AND m.user_id = u.id
...
function dwg_monitor_copy( $p_source_bug_id, $p_dest_bug_id ) {
	...
	db_param_push();
	$t_query = 'SELECT user_id FROM {dwg_monitor} WHERE dwg_id = ' . db_param();
...
function dwg_unmonitor( $p_bug_id, $p_user_id ) {
	...
	db_param_push();
	$t_query = 'DELETE FROM {dwg_monitor} WHERE dwg_id = ' . db_param();
...
function dwg_cache_columns_data( array $p_bugs, array $p_selected_columns ) {
	...
	foreach( $p_selected_columns as $t_column ) {
		...
		if( column_dwg_is_custom_field( $t_column ) ) {
			$t_cf_name = column_dwg_get_custom_field_name( $t_column );
		...
		switch( $t_column ) {
			case 'attachment_count':
				file_dwg_attachment_count_cache( $t_bug_ids );
				break;
			case 'handler_id':
			case 'creator_id':
			case 'status':
