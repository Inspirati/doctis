...
form_security_validate( 'dwg_query_store' );
...
$t_query_redirect_url = 'dwg_query_store_page.php';
...
if( is_blank( $f_query_name ) ) {
	...
	print_dwg_header_redirect( $t_query_redirect_url );
}

# mantis_filters_table.name has a length of 64. Not allowing longer.
if( !filter_dwg_name_valid_length( $f_query_name ) ) {
	$t_query_redirect_url = helper_url_combine( $t_query_redirect_url,
...
$t_filter_string = filter_dwg_serialize( $t_filter );
$t_new_row_id = filter_dwg_db_create_filter( $t_filter_string, auth_get_current_user_id(), $t_project_id, $f_query_name , $f_is_public );

form_security_purge( 'dwg_query_store' );
...
if( $t_new_row_id == -1 ) {
	...
	$t_redirect = helper_url_combine( 'view_dwg_set.php', $t_params );
