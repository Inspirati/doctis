...
$t_closed = config_get( 'dwg_closed_status_threshold', null, null, $t_bug->project_id );
$t_resolution_fixed = config_get( 'dwg_resolution_fixed_threshold', null, null, $t_bug->project_id );
...
} else if( dwg_is_readonly( $f_bug_id )
	|| !access_has_dwg_level( access_get_dwg_status_threshold( $f_new_status, $t_bug->project_id ), $f_bug_id, $t_current_user_id ) ) {
	access_denied();
}

$t_can_update_due_date = access_has_dwg_level( config_get( 'due_date_update_threshold' ), $f_bug_id );

# get new issue handler if set, otherwise default to original handler
$f_handler_id = gpc_get_int( 'handler_id', $t_bug->handler_id );

if( config_get( 'dwg_assigned_status' ) == $f_new_status ) {
	$t_bug_sponsored = config_get( 'enable_sponsorship' )
		&& sponsorship_get_amount( sponsorship_get_all_ids( $f_bug_id ) ) > 0;
	if( $t_bug_sponsored && !access_has_dwg_level( config_get( 'assign_sponsored_dwgs_threshold' ), $f_bug_id ) ) {
	...
	if( $f_handler_id != NO_USER ) {
		...
		if( !access_has_project_level( config_get( 'handle_dwg_threshold' ), $t_bug->project_id, $f_handler_id ) ) {
			trigger_error( ERROR_HANDLER_ACCESS_TOO_LOW, ERROR );
		}
		if( $t_bug_sponsored && !access_has_project_level( config_get( 'handle_sponsored_dwgs_threshold' ), $t_bug->project_id, $f_handler_id ) ) {
...
<div class="col-md-12 col-xs-12">
	...
	<form id="bug-change-status-form" name="dwg_change_status_form" method="post" action="dwg_update.php">
	...
	<div class="widget-header widget-header-small">
		<h4 class="widget-title lighter">
			<?php echo lang_get( $t_status_label . '_dwg_title' ) ?>
		</h4>
	</div>
		...
		<thead>
			<?php
				if( $f_new_status >= $t_resolved ) {
					if( dwg_relationship_can_resolve_dwg( $f_bug_id ) == false ) {
						if( OFF == config_get( 'allow_parent_of_unresolved_to_close' ) ) {
							trigger_error( ERROR_DWG_RESOLVE_DEPENDANTS_BLOCKING, ERROR );
						}
						echo '<tr><td colspan="2">' . lang_get( 'relationship_warning_blocking_dwgs_not_resolved_2' ) . '</td></tr>';
					}
				}
			?>
		</thead>
...
<?php
		if( $t_resolution != config_get( 'dwg_duplicate_resolution' ) ) {
?>
<!-- Duplicate ID -->
			<tr>
				<th class="category">
...
<?php
	...
	if( access_has_dwg_level( config_get( 'update_dwg_assign_threshold', config_get( 'update_dwg_threshold' ) ), $f_bug_id ) ) {
		$t_suggested_handler_id = $t_bug->handler_id;

		if( $t_suggested_handler_id == NO_USER && access_has_dwg_level( config_get( 'handle_dwg_threshold' ), $f_bug_id ) ) {
	...
	foreach( $t_related_custom_field_ids as $t_id ) {
		...
		if( !$t_show_custom_field ) {
			if( in_array( $t_custom_status_label, array( 'resolved', 'closed' ) ) && $t_display ) {
				...
				$t_plugin_Event_result = event_signal( 'EVENT_UPDATE_DWG_SHOW_CUSTOM_FIELD', array( $t_bug, $t_id ) );
...
<?php
	...
	if( ( $f_new_status >= $t_resolved ) ) {
		if( version_should_show_product_version( $t_bug->project_id )
			&& !dwg_is_readonly( $f_bug_id )
			&& access_has_dwg_level( config_get( 'update_dwg_threshold' ), $f_bug_id )
		) {
?>
			<!-- Fixed in Version -->
			<tr>
...
<?php
	...
	event_signal( 'EVENT_UPDATE_DWG_STATUS_FORM', array( $f_bug_id, $f_new_status ) );

	if( $f_change_type == DWG_UPDATE_TYPE_REOPEN ) {
?>
	<!-- Bug was re-opened -->
<?php
		printf( '	<input type="hidden" name="resolution" value="%s" />' . "\n", config_get( 'dwg_reopen_resolution' ) );
	}
?>
<?php
	...
	if( access_has_dwg_level( config_get( 'private_dwgnote_threshold' ), $f_bug_id ) ) { ?>
			<tr>
				<th class="category">
...
<?php
	if( config_get( 'time_tracking_enabled' )
		&& access_has_dwg_level( config_get( 'private_dwgnote_threshold' ), $f_bug_id )
		&& access_has_dwg_level( config_get( 'time_tracking_edit_threshold' ), $f_bug_id )
	) {
	?>
			<tr>
				<th class="category">
...
<?php
	...
	event_signal( 'EVENT_DWGNOTE_ADD_FORM', array( $f_bug_id ) );
?>
...
<div class="widget-toolbox padding-8 clearfix">
	...
	<input type="submit" class="btn btn-primary btn-white btn-round" value="<?php echo lang_get( $t_status_label . '_dwg_button' ) ?>" />
</div>
</div>
</div>
</fieldset>
...
<?php
define( 'DWG_VIEW_INC_ALLOW', true );
include( __DIR__ . '/dwg_view_inc.php' );
