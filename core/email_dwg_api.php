 ...
 *
 * @todo yarick123: email_dwg_collect_recipients(...) will be completely rewritten to provide additional information such as language, user access,..
 ...
 */
function email_dwg_collect_recipients( $p_bug_id, $p_notify_type, array $p_extra_user_ids_to_email = array(), $p_bugnote_id = null ) {
	$t_recipients = array();
	...
	foreach ( $p_extra_user_ids_to_email as $t_user_id ) {
		if ( $t_explicit_enabled ) {
			...
			log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, add @U%d (explicitly specified)', $p_bug_id, $t_user_id );
		} else {
	...
	if( $t_handler_id > 0 ) {
		if( ON == email_notify_flag( $p_notify_type, 'handler' ) ) {
			...
			log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, add @U%d (handler)', $p_bug_id, $t_handler_id );
		} else {
			log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, skip @U%d (handler disabled)', $p_bug_id, $t_handler_id );
		}
	}
	...
	while( $t_row = db_fetch_array( $t_result ) ) {
		...
		if ( $t_monitoring_enabled ) {
			$t_recipients[$t_user_id] = true;
			log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, add @U%d (monitoring)', $p_bug_id, $t_user_id );
		} else {
			log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, skip @U%d (monitoring disabled)', $p_bug_id, $t_user_id );
		}
	}
	...
	if( ON == email_notify_flag( $p_notify_type, 'category' ) ) {
		...
		if( $t_category_id > 0 ) {
			...
			if( $t_category_assigned_to > 0 ) {
				...
				log_event( LOG_EMAIL_RECIPIENT, sprintf( 'Document = #%d, add Category Owner = @U%d', $p_bug_id, $t_category_assigned_to ) );
			}
		}
	}
	...
	while( $t_row = db_fetch_array( $t_result ) ) {
		...
		if ( $t_notes_enabled ) {
			...
			log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, skip @U%d (note author disabled)', $p_bug_id, $t_user_id );
		}
	}

	# add project users who meet the thresholds
	$t_bug_is_private = dwg_get_field( $p_bug_id, 'view_state' ) == VS_PRIVATE;
	...
	foreach( $t_threshold_users as $t_user ) {
		if( $t_user['access_level'] <= $t_threshold_max ) {
			if( !$t_bug_is_private || access_compare_level( $t_user['access_level'], config_get( 'private_dwg_threshold' ) ) ) {
				$t_recipients[$t_user['id']] = true;
				log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, add @U%d (based on access level)', $p_bug_id, $t_user['id'] );
			}
		}
	}
		...
		foreach( $t_recipients_include_data2 as $t_recipients_included ) {
			...
			if( is_array( $t_recipients_included ) ) {
				foreach( $t_recipients_included as $t_user_id ) {
					$t_recipients[$t_user_id] = true;
					log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, add @U%d (by %s plugin)', $p_bug_id, $t_user_id, $t_plugin );
				}
			}
		}
	}
	...
	foreach( $t_recipients as $t_id => $t_ignore ) {
		...
		if( ( auth_get_current_user_id() == $t_id ) && ( OFF == config_get( 'email_receive_own' ) ) ) {
			log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, drop @U%d (own action)', $p_bug_id, $t_id );
			continue;
		}

		# Eliminate users who don't exist anymore or who are disabled
		if( !user_exists( $t_id ) || !user_is_enabled( $t_id ) ) {
			log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, drop @U%d (user disabled)', $p_bug_id, $t_id );
			continue;
		}
		...
		if( $t_pref_field ) {
			...
			if( OFF == $t_notify ) {
				log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, drop @U%d (pref %s off)', $p_bug_id, $t_id, $t_pref_field );
				continue;
			} else {
		...
		if(   !access_has_dwg_level( $t_view_bug_threshold, $p_bug_id, $t_id )
		   || (   $p_bugnote_id
			   && !access_has_dwgnote_level( $t_view_bug_threshold, $p_bugnote_id, $t_id )
			  )
		) {
			log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, drop @U%d (access level)', $p_bug_id, $t_id );
			continue;
		}
		...
		foreach( $t_recipient_exclude_data as $t_plugin => $t_recipient_exclude_data2 ) {
			foreach( $t_recipient_exclude_data2 as $t_recipient_excluded ) {
				...
				if( $t_recipient_excluded ) {
					$t_exclude = true;
					log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, drop @U%d (by %s plugin)', $p_bug_id, $t_id, $t_plugin );
				}
			}
		}
		...
		if( is_blank( $t_email ) ) {
			log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, drop @U%d (no email address)', $p_bug_id, $t_id );
		} else {
 ...
 * @throws ClientException
 */
function email_dwg_generic( $p_bug_id, $p_notify_type, $p_message_id = null, array $p_header_optional_params = [], array $p_extra_user_ids_to_email = array() ) {
	# @todo yarick123: email_dwg_collect_recipients(...) will be completely rewritten to provide additional information such as language, user access,..
	# @todo yarick123:sort recipients list by language to reduce switches between different languages
	$t_recipients = email_dwg_collect_recipients( $p_bug_id, $p_notify_type, $p_extra_user_ids_to_email );
	email_dwg_generic_to_recipients( $p_bug_id, $p_notify_type, $t_recipients, $p_message_id, $p_header_optional_params );
}
 ...
 * @throws ClientException
 */
function email_dwg_generic_to_recipients( int $p_bug_id, string $p_notify_type, array $p_recipients, $p_message_id = null, array $p_header_optional_params = [] ) {
	...
	foreach( $p_recipients as $t_user_id => $t_user_email ) {
		log_event( LOG_EMAIL_VERBOSE, 'Document = #%d, Type = %s, Msg = \'%s\', User = @U%d, Email = \'%s\'.', $p_bug_id, $p_notify_type, $p_message_id, $t_user_id, $t_user_email );
		...
		$t_visible_bug_data = email_build_visible_dwg_data( $t_user_id, $p_bug_id, $p_message_id );
		email_dwg_info_to_one_user( $t_visible_bug_data, $p_message_id, $t_user_id, $p_header_optional_params );
 ...
 * @throws ClientException
 */
function email_dwg_monitor_added( $p_bug_id, $p_user_id ) {
	log_event( LOG_EMAIL, 'Document #%d monitored by user @U%d', $p_bug_id, $p_user_id );

	$t_opt = array();
	$t_opt[] = dwg_format_id( $p_bug_id );
	$t_opt[] = user_get_name( $p_user_id );

	email_dwg_generic( $p_bug_id, 'monitor', 'email_notification_title_for_action_dwg_monitor', $t_opt, array( $p_user_id ) );
}
 ...
 * @throws ClientException
 */
function email_dwg_relationship_added( $p_bug_id, $p_related_bug_id, $p_rel_type, $p_email_for_source ) {
	...
	log_event(
		LOG_EMAIL,
		'Document #%d relationship added to Document #%d (relationship type %s)',
		$p_bug_id,
		$p_related_bug_id,
	...
	if( $p_email_for_source ) {
		...
		email_dwg_relationship_send( $p_bug_id, $p_related_bug_id, $t_message_id );
	}
	...
	email_dwg_relationship_send( $p_related_bug_id, $p_bug_id, $t_message_id );
}
 ...
 * @access private
 */
function email_filter_recipients_for_dwg( $p_bug_id, array $p_recipients ) {
	$t_view_bug_threshold = config_get( 'view_dwg_threshold' );

	return array_filter( $p_recipients,
		function( $t_recipient_id ) use ( $t_view_bug_threshold, $p_bug_id ) {
			return access_has_dwg_level( $t_view_bug_threshold, $p_bug_id, $t_recipient_id );
		},
 ...
 * @throws ClientException
 */
function email_dwg_relationship_send( int $p_bug_id, int $p_related_bug_id, $p_message_id ) {
	$t_recipients = email_dwg_collect_recipients( $p_bug_id, 'relation' );

	# Recipient has to have access to both bugs to get the notification.
	$t_recipients = email_filter_recipients_for_dwg( $p_bug_id, $t_recipients );
	$t_recipients = email_filter_recipients_for_dwg( $p_related_bug_id, $t_recipients );

	$t_opt = [ dwg_format_id( $p_related_bug_id ) ];

	email_dwg_generic_to_recipients( $p_bug_id, 'relation', $t_recipients, $p_message_id, $t_opt );
}
 ...
 * @throws ClientException
 */
function email_dwg_relationship_deleted( $p_bug_id, $p_related_bug_id, $p_rel_type, $p_skip_email_for_issue_id = 0 ) {
	global $g_relationships;
	if( !isset( $g_relationships[$p_rel_type] ) ) {
		trigger_error( ERROR_RELATIONSHIP_NOT_FOUND, ERROR );
	}
	...
	log_event(
		LOG_EMAIL,
		'Document #%d relationship to Document #%d (relationship type %s) deleted.',
		$p_bug_id,
		$p_related_bug_id,
		$g_relationships[$p_rel_type]['#description'] );
	...
	# Destination issue email notification. Should use the relationship reverse message
	if( $p_bug_id != $p_related_bug_id && dwg_exists( $p_related_bug_id) ) {
 ...
 * @throws ClientException
 */
function email_dwg_relationship_child_resolved( $p_bug_id ) {
	email_dwg_relationship_child_resolved_closed( $p_bug_id, 'email_notification_title_for_action_dwg_relationship_child_resolved' );
}
 ...
 * @throws ClientException
 */
function email_dwg_relationship_child_closed( $p_bug_id ) {
	email_dwg_relationship_child_resolved_closed( $p_bug_id, 'email_notification_title_for_action_dwg_relationship_child_closed' );
}
	...
	for( $i = 0;$i < $t_relationship_count;$i++ ) {
		if( $t_relationship[$i]->type == DWG_DEPENDANT ) {
			...
			if( $t_status < config_get( 'dwg_resolved_status_threshold' ) ) {
				...
				$t_opt = array();
				$t_opt[] = dwg_format_id( $p_bug_id );
				email_dwg_generic( $t_src_bug_id, 'handler', $p_message_id, $t_opt );
			}
		}
	}
}
 ...
 * @throws ClientException
 */
function email_dwg_sponsorship_added( $p_bug_id ) {
	log_event( LOG_EMAIL, sprintf( 'Document #%d sponsorship added', $p_bug_id ) );
	email_dwg_generic( $p_bug_id, 'sponsor', 'email_notification_title_for_action_dwg_sponsorship_added' );
}
 ...
 * @throws ClientException
 */
function email_dwg_sponsorship_updated( $p_bug_id ) {
	log_event( LOG_EMAIL, sprintf( 'Document #%d sponsorship updated', $p_bug_id ) );
	email_dwg_generic( $p_bug_id, 'sponsor', 'email_notification_title_for_action_dwg_sponsorship_updated' );
}
 ...
 * @throws ClientException
 */
function email_dwg_sponsorship_deleted( $p_bug_id ) {
	log_event( LOG_EMAIL, sprintf( 'Document #%d sponsorship removed', $p_bug_id ) );
	email_dwg_generic( $p_bug_id, 'sponsor', 'email_notification_title_for_action_dwg_sponsorship_deleted' );
}
 ...
 * @throws ClientException
 */
function email_dwgnote_add( $p_bugnote_id, $p_files = array(), $p_exclude_user_ids = array() ) {
	if( OFF == config_get( 'enable_email_notification' ) ) {
	...
	$t_message_id = 'email_notification_title_for_action_dwg_bugnote_submitted';
	...
	foreach( $t_recipients as $t_user_id => $t_user_email ) {
		if( in_array( $t_user_id, $p_exclude_user_ids ) ) {
			log_event( LOG_EMAIL_RECIPIENT, 'Document = #%d, Note = ~%d, Type = %s, Msg = \'%s\', User = @U%d excluded, Email = \'%s\'.',
				$t_bugnote->dwg_id, $p_bugnote_id, 'bugnote', 'email_notification_title_for_action_dwg_bugnote_submitted', $t_user_id, $t_user_email );
			continue;
		}
		...
		$t_message = lang_get( 'email_notification_title_for_action_dwg_dwgnote_submitted' ) . "\n\n";
 ...
 * @throws ClientException
 */
function email_dwg_resolved( $p_bug_id ) {
	log_event( LOG_EMAIL, sprintf( 'Document #%d resolved', $p_bug_id ) );
	email_dwg_generic( $p_bug_id, 'resolved', 'email_notification_title_for_status_dwg_resolved' );
}
...
function email_dwg_close( $p_bug_id ) {
	...
	email_dwg_generic( $p_bug_id, 'closed', 'email_notification_title_for_status_dwg_closed' );
}
 ...
 * @throws ClientException
 */
function email_dwg_owner_changed($p_bug_id, $p_prev_handler_id, $p_new_handler_id ) {
	if ( $p_prev_handler_id == 0 && $p_new_handler_id != 0 ) {
		log_event( LOG_EMAIL, sprintf( 'Document #%d assigned to user @U%d.', $p_bug_id, $p_new_handler_id ) );
	} else if ( $p_prev_handler_id != 0 && $p_new_handler_id == 0 ) {
		log_event( LOG_EMAIL, sprintf( 'Document #%d is no longer assigned to @U%d.', $p_bug_id, $p_prev_handler_id ) );
	} else {
		log_event(
			LOG_EMAIL,
			sprintf(
				'Document #%d is assigned to @U%d instead of @U%d.',
				$p_bug_id,
				$p_new_handler_id,
				$p_prev_handler_id )
		);
	...
	email_dwg_generic( $p_bug_id, 'owner', $t_message_id, /* headers */ [], $t_extra_user_ids_to_email );
}
...
function email_dwg_status_changed( $p_bug_id, $p_new_status_label ) {
	...
	email_dwg_generic( $p_bug_id, $p_new_status_label, 'email_notification_title_for_status_dwg_' . $p_new_status_label );
}
 ...
 * @throws ClientException
 */
function email_dwg_build_subject( $p_bug_id ) {
 ...
 * @throws ClientException
 */
function email_dwg_reminder( $p_recipients, $p_bug_id, $p_message ) {
	if( OFF == config_get( 'enable_email_notification' ) ) {
		return array();
	}
	...
	$t_subject = email_dwg_build_subject( $p_bug_id );
 ...
 * @throws ClientException
 */
function email_dwg_user_mention( $p_bug_id, $p_mention_user_ids, $p_message, $p_removed_mention_user_ids = array() ) {
	if( OFF == config_get( 'enable_email_notification' ) ) {
		log_event( LOG_EMAIL_VERBOSE, 'email notifications disabled.' );
		return array();
	}
 ...
 * @throws ClientException
 */
function email_dwg_info_to_one_user( array $p_visible_bug_data, string $p_message_id, int $p_user_id, array $p_header_optional_params = [] ) {
	...
	# build subject
	$t_subject = email_dwg_build_subject( $p_visible_bug_data['email_dwg'] );
	...
	$t_message .= email_format_dwg_message( $p_visible_bug_data );

	# build headers
	$t_bug_id = $p_visible_bug_data['email_dwg'];
	...
	if( $p_message_id == 'email_notification_title_for_action_dwg_bug_submitted' ) {
 ...
 */
function email_format_dwgnote( $p_bugnote, $p_project_id, $p_show_time_tracking, $p_horizontal_separator, $p_date_format = null ) {
	...
	if( user_exists( $p_bugnote->creator_id ) ) {
		$t_access_level = access_get_project_level( $p_project_id, $p_bugnote->creator_id );
	...
	$t_string = ' (' . $t_formatted_bugnote_id . ') ' . user_get_name( $p_bugnote->creator_id ) .
 ...
 * @return string
 */
function email_format_dwg_message( array $p_visible_bug_data ) {
	...
	$t_message .= email_dwg_format_attribute( $p_visible_bug_data, 'email_creator' );
	$t_message .= email_dwg_format_attribute( $p_visible_bug_data, 'email_handler' );
	$t_message .= $t_email_separator1 . " \n";
	$t_message .= email_dwg_format_attribute( $p_visible_bug_data, 'email_project' );
	$t_message .= email_dwg_format_attribute( $p_visible_bug_data, 'email_dwg' );
	$t_message .= email_dwg_format_attribute( $p_visible_bug_data, 'email_category' );
	...
	if ( isset( $p_visible_bug_data[ 'email_priority' ] ) ) {
		...
		$t_message .= email_dwg_format_attribute( $p_visible_bug_data, 'email_priority' );
	}
	...
	$t_message .= email_dwg_format_attribute( $p_visible_bug_data, 'email_date_submitted' );
	$t_message .= email_dwg_format_attribute( $p_visible_bug_data, 'email_last_modified' );

	if( isset( $p_visible_bug_data['email_due_date'] ) ) {
		$t_message .= email_dwg_format_attribute( $p_visible_bug_data, 'email_due_date' );
	}
 ...
 * @return string
 */
function email_dwg_format_attribute( array $p_visible_bug_data, $p_attribute_id ) {
 ...
 * @throws ClientException
 */
function email_build_visible_dwg_data( $p_user_id, $p_bug_id, $p_message_id ) {
	...
	$t_bug_data['email_creator'] = user_get_name( $t_row['creator_id'] );
	...
	if( ( config_get( 'enable_sponsorship' ) == ON ) && ( access_has_dwg_level( config_get( 'view_sponsorship_total_threshold' ), $p_bug_id, $p_user_id ) ) ) {
		$t_sponsorship_ids = sponsorship_get_all_ids( $p_bug_id );
		$t_bug_data['sponsorship_total'] = sponsorship_get_amount( $t_sponsorship_ids );

		if( access_has_dwg_level( config_get( 'view_sponsorship_details_threshold' ), $p_bug_id, $p_user_id ) ) {
	...
	$t_bug_data['relations'] = email_dwg_relationship_get_summary_text( $p_bug_id );
 ...
 * @throws ClientException
 */
function email_dwg_relationship_get_summary_text( $p_bug_id ) {
	...
	for( $i = 0; $i < $t_relationship_all_count; $i++ ) {
		$t_summary .= email_dwg_relationship_get_details( $p_bug_id, $t_relationship_all[$i] );
	}
