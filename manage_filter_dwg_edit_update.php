...
form_security_validate( 'manage_filter_dwg_edit_update' );
...
if( $f_filter_name != filter_dwg_get_field( $f_filter_id, 'name' ) ) {
...
$t_editable = filter_dwg_db_can_delete_filter( $f_filter_id );
if( !$t_editable ) {
	access_denied();
}
...
if( empty( $t_errors ) ) {
	filter_dwg_db_update_filter( $f_filter_id, filter_dwg_serialize( $t_filter ), $f_project_id, $f_is_public, $f_filter_name );
	print_dwg_header_redirect( 'manage_filter_dwg_page.php' );
}
...
html_operation_failure(
				helper_mantis_url( 'manage_filter_dwg_edit_page.php?filter_id=' . $f_filter_id ),
				$t_error_html
			);
