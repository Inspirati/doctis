...
# Cache of dwgnotes arrays related to a bug, indexed by dwg_id.
...
/**
 * Dwgnote Data Structure Definition
 */
class DwgnoteData {
	/**
	 * Bugnote ID
 ...
 * @access public
 */
function dwgnote_exists( $p_bugnote_id ) {
	$c_bugnote_id = (int)$p_bugnote_id;

	global $g_cache_dwgnotes_by_id;
	if( isset( $g_cache_dwgnotes_by_id[$c_bugnote_id] ) ) {
		return true;
	}
	...
	$t_query = 'SELECT b.*, t.note
				FROM {dwgnote} b
				LEFT JOIN {dwgnote_text} t ON b.dwgnote_text_id = t.id
				WHERE b.id = ' . db_param();
	...
	$t_bugnote = dwgnote_row_to_object( $t_row );
	dwgnote_cache( $t_bugnote );
	return true;
}
 ...
 * @access public
 */
function dwgnote_ensure_exists( $p_bugnote_id ) {
	if( !dwgnote_exists( $p_bugnote_id ) ) {
		throw new ClientException(
			"Document note #$p_bugnote_id not found",
			ERROR_DWGNOTE_NOT_FOUND,
			array( $p_bugnote_id ) );
	}
}
 ...
 * @access public
 */
function dwgnote_add( $p_bug_id, $p_bugnote_text, $p_time_tracking = '0:00', $p_private = false, $p_type = BUGNOTE, $p_attr = '', $p_user_id = null, $p_send_email = true, $p_date_submitted = 0, $p_last_modified = 0, $p_skip_bug_update = false, $p_log_history = true, $p_trigger_event = true ) {
	...
	# Event integration
	$t_bugnote_text = event_signal( 'EVENT_DWGNOTE_DATA', $p_bugnote_text, $c_bug_id );
	...
	$t_bugnote_text_id = db_insert_id( db_get_table( 'dwgnote_text' ) );
	...
	if( $p_private && access_has_dwg_level( config_get( 'set_view_status_threshold' ), $p_bug_id, $p_user_id ) ) {
	...
	$t_query = 'INSERT INTO {dwgnote}
			(dwg_id, creator_id, dwgnote_text_id, view_state, date_submitted, last_modified, note_type, note_attr, time_tracking)
		VALUES ('
	...
	# get bugnote id
	$t_bugnote_id = db_insert_id( db_get_table( 'dwgnote' ) );
	...
	# Event integration
	if( $p_trigger_event ) {
		event_signal( 'EVENT_DWGNOTE_ADD', array( $p_bug_id, $t_bugnote_id, array() ) );
	}
	...
	if( $p_send_email && !is_blank( $t_bugnote_text ) ) {
		email_dwgnote_add( $t_bugnote_id );
	}
 ...
 * @access public
 */
function dwgnote_process_mentions( $p_bug_id, $p_bugnote_id, $p_bugnote_text ) {
	# Process the mentions that have access to the issue note
	$t_mentioned_user_ids = mention_get_users( $p_bugnote_text );
	$t_filtered_mentioned_user_ids = access_has_dwgnote_level_filter(
	...
	return mention_dwg_process_user_mentions(
		$p_bug_id,
 ...
 * @access public
 */
function dwgnote_delete( $p_bugnote_id ) {
	$t_bug_id = dwgnote_get_field( $p_bugnote_id, 'dwg_id' );
	$t_bugnote_text_id = dwgnote_get_field( $p_bugnote_id, 'dwgnote_text_id' );
...
function dwgnote_delete_all( $p_bug_id ) {
	...
	db_param_push();
	$t_query = 'SELECT dwgnote_text_id FROM {dwgnote} WHERE dwg_id=' . db_param();
	$t_result = db_query( $t_query, array( (int)$p_bug_id ) );
	while( $t_row = db_fetch_array( $t_result ) ) {
		$t_bugnote_text_id = $t_row['dwgnote_text_id'];
	...
	db_param_push();
	$t_query = 'DELETE FROM {dwgnote} WHERE dwg_id=' . db_param();
 ...
 * @access public
 */
function dwgnote_get_text( $p_bugnote_id ) {
	$t_bugnote_text_id = dwgnote_get_field( $p_bugnote_id, 'dwgnote_text_id' );
 ...
 * @access public
 */
function dwgnote_get_field( $p_bugnote_id, $p_field_name ) {
...
function dwgnote_get_all_visible_dwgnotes( $p_bug_id, $p_user_bugnote_order, $p_user_bugnote_limit, $p_user_id = null ) {
	...
	for( $i = 0; ( $i < $t_bugnote_count ) && ( $t_bugnotes_found < $t_bugnote_limit ); $i++ ) {
		...
		if( $t_private_bugnote_visible || $t_bugnote->creator_id == $t_user_id || ( VS_PUBLIC == $t_bugnote->view_state ) ) {
...
function dwgnote_get_all_visible_as_string( $p_bug_id, $p_user_bugnote_order, $p_user_bugnote_limit, $p_user_id = null ) {
	...
	foreach( $t_notes as $t_note ) {
		$t_note_string = '@' . user_get_name( $t_note->creator_id );
 ...
 *
 * @param array $p_row The bugnote row (including dwgnote_text note)
 *
...
function dwgnote_row_to_object( array $p_row ) {
	...
	$t_bugnote->dwg_id = (int)$p_row['dwg_id'];
	$t_bugnote->dwgnote_text_id = (int)$p_row['dwgnote_text_id'];
...
function dwgnote_get_all_dwgnotes( $p_bug_id ) {
	...
	if( !isset( $g_cache_dwgnotes_by_bug_id[(int)$p_bug_id] ) ) {
		...
		$g_cache_dwgnotes_by_bug_id[(int)$p_bug_id] = $t_bugnotes;
	}

	return $g_cache_dwgnotes_by_bug_id[(int)$p_bug_id];
}
...
function dwgnote_get( $p_bugnote_id ) {
	...
	dwgnote_ensure_exists( $p_bugnote_id );

	global $g_cache_dwgnotes_by_id;

	# Return the object from the cache, fetched above.
	if( isset( $g_cache_dwgnotes_by_id[(int)$p_bugnote_id] ) ) {
		return $g_cache_dwgnotes_by_id[(int)$p_bugnote_id];
	}

	# if we reached here something is wrong, trigger an error.
	trigger_error( ERROR_DWGNOTE_NOT_FOUND, ERROR );
}
 ...
 * @access public
 */
function dwgnote_set_time_tracking( $p_bugnote_id, $p_time_tracking ) {
 ...
 * @access public
 */
function dwgnote_set_text( $p_bugnote_id, $p_bugnote_text ) {
	$t_old_text = dwgnote_get_text( $p_bugnote_id );
	...
	if( dwg_revision_count( $t_bug_id, REV_DWGNOTE, $p_bugnote_id ) < 1 ) {
		$t_user_id = dwgnote_get_field( $p_bugnote_id, 'creator_id' );
		$t_timestamp = dwgnote_get_field( $p_bugnote_id, 'last_modified' );
		dwg_revision_add( $t_bug_id, $t_user_id, REV_DWGNOTE, $t_old_text, $p_bugnote_id, $t_timestamp );
	}
	...
	dwgnote_date_update( $p_bugnote_id );
	dwg_update_date( $t_bug_id );
 ...
 * @access public
 */
function dwgnote_set_view_state( $p_bugnote_id, $p_private ) {
	$t_bug_id = dwgnote_get_field( $p_bugnote_id, 'dwg_id' );
 ...
 * @access public
 */
function dwgnote_format_id( $p_bugnote_id ) {
	$t_padding = config_get( 'display_dwgnote_padding' );
 ...
 * @access public
 */
function dwgnote_stats_get_events_array( $p_bug_id, $p_from, $p_to ) {
 ...
 * @access public
 */
function dwgnote_clear_cache( $p_bugnote_id = null ) {
	global $g_cache_dwgnotes_by_id, $g_cache_dwgnotes_by_bug_id;
	...
	if( null === $p_bugnote_id ) {
		...
		if( isset( $g_cache_dwgnotes_by_id[$p_bugnote_id] ) ) {
			$t_note_obj = $g_cache_dwgnotes_by_id[$p_bugnote_id];
			unset($g_cache_dwgnotes_by_id[$p_bugnote_id]);

			# Clear the bug-level cache for the bugnote's parent bug
			dwgnote_clear_bug_cache( $t_note_obj->bug_id );
		}
	}
 ...
 * @access public
 */
function dwgnote_clear_bug_cache( $p_bug_id = null ) {
	global $g_cache_dwgnotes_by_bug_id, $g_cache_dwgnotes_by_id;
	...
	if( null === $p_bug_id ) {
		...
		if( isset( $g_cache_dwgnotes_by_bug_id[(int)$p_bug_id] ) ) {
			foreach( $g_cache_dwgnotes_by_bug_id[(int)$p_bug_id] as $t_note_obj ) {
				unset( $g_cache_dwgnotes_by_id[(int)$t_note_obj->id] );
			}
			unset( $g_cache_dwgnotes_by_bug_id[(int)$p_bug_id] );
		}
	}
