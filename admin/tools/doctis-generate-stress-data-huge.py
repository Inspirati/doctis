...
N_PROJECTS       = 50    # top-level projects
N_SUB            = 12    # sub-projects per top-level project
N_SUBSUB         = 3    # sub-sub-projects per sub-project (set 0 to omit)
N_DOCS_PER_PRJ   = 12    # documents at EACH project level (top + sub + subsub)
N_ISSUES_PER_DOC = 50    # bug/issue rows per document
N_USERS          = 120   # stress users to add (above the existing sample accounts)
N_LICENSES       = 50   # stress licenses to add (above the existing sample licenses)
N_NEWS           = 500   # news stories spread across top-level projects
