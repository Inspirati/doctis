 ...
 * @param integer $p_dwgcount The bug count of $p_status issues.
 * @return void
 */
function summary_helper_build_dwgcount( &$p_cache, $p_key, $p_status, $p_dwgcount ) {
	...
	if( $t_closed_val <= $p_status ) {
		if( isset( $p_cache[$p_key]['closed'] ) ) {
			$p_cache[$p_key]['closed'] += $p_dwgcount;
		} else {
			$p_cache[$p_key]['closed'] = $p_dwgcount;
		}
	} else if( $t_resolved_val <= $p_status ) {
		if( isset( $p_cache[$p_key]['resolved'] ) ) {
			$p_cache[$p_key]['resolved'] += $p_dwgcount;
		} else {
			$p_cache[$p_key]['resolved'] = $p_dwgcount;
		}
	} else {
		if( isset( $p_cache[$p_key]['open'] ) ) {
			$p_cache[$p_key]['open'] += $p_dwgcount;
		} else {
			$p_cache[$p_key]['open'] = $p_dwgcount;
		}
	}
...
function summary_print_by_enum( $p_enum, array $p_filter = [] ) {
	...
	while( $t_row = $t_query->fetch() ) {
		...
		$t_dwgcount = $t_row['dwgcount'];
		$t_bugs_total_count += $t_dwgcount;
		
		summary_helper_build_dwgcount( $t_cache, $t_enum, $t_status, $t_dwgcount );
	}
...
function summary_print_by_activity( array $p_filter = [] ) {
	...
	$t_sql .= ' GROUP BY h.dwg_id, b.id, b.summary, b.last_updated, b.view_state'
	...
	while( $t_row = $t_query->fetch() ) {
		...
		if( ( VS_PRIVATE == $t_row['view_state'] ) && ( false == access_has_dwg_level( $t_private_bug_threshold, $t_row['id'] ) ) ) {
			continue;
		}
...
function summary_print_by_developer( array $p_filter = [] ) {
	...
	while( $t_row = $t_query->fetch() ) {
		...
		summary_helper_build_dwgcount( $t_cache, $t_label, $t_status, $t_dwgcount );
	}
...
function summary_print_by_creator( array $p_filter = [] ) {
	...
	$t_reporters_count = 0;
	while( $t_row = $t_query->fetch() ) {
		$t_reporters[] = (int)$t_row['creator_id'];
	...
	while( $t_row = $t_query->fetch() ) {
		$t_creator_id = (int)$t_row['creator_id'];
		if( !isset( $t_reporter_stats[$t_creator_id] ) ) {
			$t_reporter_stats[$t_creator_id] = array(
				'open' => 0,
				'resolved' => 0,
				'closed' => 0,
				'total' => 0,
				'creator_id' => $t_creator_id
				);
		}
		$t_dwgcount = (int)$t_row['dwgcount'];
		$t_status = (int)$t_row['status'];
		$t_reporter_stats[$t_creator_id]['total'] += $t_dwgcount;
		if( $t_status >= $t_closed_status ) {
			$t_reporter_stats[$t_creator_id]['closed'] += $t_dwgcount;
		} elseif ( $t_status >= $t_resolved_status ) {
			$t_reporter_stats[$t_creator_id]['resolved'] += $t_dwgcount;
		} else {
			$t_reporter_stats[$t_creator_id]['open'] += $t_dwgcount;
		}
	}
...
function summary_print_by_project( array $p_projects = [], int $p_level = 0, array $p_cache = [], array $p_filter = [] ) {
	...
	if( empty( $p_cache ) ) {
		...
		while( $t_row = $t_query->fetch() ) {
			...
			summary_helper_build_dwgcount( $p_cache, $t_project_id, $t_status, $t_dwgcount );
		}
...
function summary_print_developer_resolution( $p_resolution_enum_string, array $p_filter = [] ) {
	...
	while( $t_arr ) {
		...
		$t_handler_res_arr[$t_arr['handler_id']][$t_arr['resolution']] += $t_arr['dwgcount'];
		$t_handler_res_arr[$t_arr['handler_id']]['total'] += $t_arr['dwgcount'];
...
function summary_print_reporter_resolution( $p_resolution_enum_string, array $p_filter = [] ) {
	...
	$t_reporter_dwgcount_arr = array();
	$t_arr = $t_query->fetch();
	while( $t_arr ) {
		if( !isset( $t_reporter_res_arr[$t_arr['creator_id']] ) ) {
			$t_reporter_res_arr[$t_arr['creator_id']] = array();
			$t_reporter_dwgcount_arr[$t_arr['creator_id']] = 0;
		}
		if( !isset( $t_reporter_res_arr[$t_arr['creator_id']][$t_arr['resolution']] ) ) {
			$t_reporter_res_arr[$t_arr['creator_id']][$t_arr['resolution']] = 0;
		}
		$t_reporter_res_arr[$t_arr['creator_id']][$t_arr['resolution']] += $t_arr['dwgcount'];
		$t_reporter_dwgcount_arr[$t_arr['creator_id']] += $t_arr['dwgcount'];

		$t_arr = $t_query->fetch();
	}

	# Sort our total bug count array so that the reporters with the highest number of bugs are listed first,
	arsort( $t_reporter_dwgcount_arr );
	...
	# We now have a multi dimensional array of users and resolutions, with the value of each resolution for each user
	foreach( $t_reporter_dwgcount_arr as $t_creator_id => $t_total_user_bugs ) {
		...
		if( $t_total_user_bugs > 0 ) {
			$t_arr2 = $t_reporter_res_arr[$t_creator_id];

			echo '<tr>';
			$t_row_count++;
			echo '<td>';
			echo string_display_line( user_get_name( $t_creator_id ) );
			echo "</td>\n";
			...
			for( $j = 0;$j < $t_enum_res_count;$j++ ) {
				...
				if( 0 < $t_res_bug_count ) {
					$t_bug_link = $t_link_prefix .
						'&amp;' . FILTER_PROPERTY_CREATOR_ID . '=' . $t_creator_id .
						'&amp;' . FILTER_PROPERTY_RESOLUTION . '=' . $c_res_s[$j] .
						'&amp;' . FILTER_PROPERTY_HIDE_STATUS . '=' . META_FILTER_NONE;
					echo '<a class="subtle" href="' . $t_bug_link . '">' . $t_res_bug_count . '</a>';
				} else {
...
// 	$t_arr = $t_query->fetch();
// 	while( $t_arr ) {
// 		if( !isset( $t_reporter_ressev_arr[$t_arr['creator_id']] ) ) {
// 			$t_reporter_ressev_arr[$t_arr['creator_id']] = array();
// 			$t_reporter_dwgcount_arr[$t_arr['creator_id']] = 0;
// 		}
// 		if( !isset( $t_reporter_ressev_arr[$t_arr['creator_id']][$t_arr['severity']] ) ) {
// 			$t_reporter_ressev_arr[$t_arr['creator_id']][$t_arr['severity']] = array();
// 			$t_reporter_ressev_arr[$t_arr['creator_id']][$t_arr['severity']]['total'] = 0;
// 		}
// 		if( !isset( $t_reporter_ressev_arr[$t_arr['creator_id']][$t_arr['severity']][$t_arr['resolution']] ) ) {
// 			$t_reporter_ressev_arr[$t_arr['creator_id']][$t_arr['severity']][$t_arr['resolution']] = 0;
// 		}
// 		$t_reporter_ressev_arr[$t_arr['creator_id']][$t_arr['severity']][$t_arr['resolution']] += $t_arr['dwgcount'];
// 		$t_reporter_ressev_arr[$t_arr['creator_id']][$t_arr['severity']]['total'] += $t_arr['dwgcount'];
// 		$t_reporter_dwgcount_arr[$t_arr['creator_id']] += $t_arr['dwgcount'];
...
// 		# users that aren't actually reporters...
// 		if( $t_total_user_bugs > 0 ) {
// 			$t_arr2 = $t_reporter_ressev_arr[$t_creator_id];
