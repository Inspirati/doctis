 ...
 * @return void
 */
function tag_dwg_cache_rows( array $p_tag_ids ) {
	global $g_cache_tags;
 ...
 *
 * @global array $g_cache_dwg_tags
 * @param array $p_bug_ids Array of bug ids.
 *
 * @return void
 */
function tag_dwg_cache_bug_tag_rows( array $p_bug_ids ) {
	global $g_cache_dwg_tags;

	$t_ids_to_search = array();
	foreach( $p_bug_ids as $t_id ) {
		if( !isset( $g_cache_dwg_tags[(int)$t_id]) ) {
	...
	while( $t_row = db_fetch_array( $t_result ) ) {
		...
		if( !isset( $g_cache_dwg_tags[$c_bug_id] ) ) {
			$g_cache_dwg_tags[$c_bug_id] = $t_has_tags? array() : false;
		}
		if( $t_has_tags ) {
			$c_tag_id = (int)$t_row['tag_id'];
			$g_cache_dwg_tags[$c_bug_id][$c_tag_id] = $t_row;
 ...
 * @return void
 */
function tag_dwg_clear_cache_bug_tags( $p_bug_id = null ) {
	global $g_cache_dwg_tags;

	if( null === $p_bug_id ) {
		$g_cache_dwg_tags = array();
	} else {
		if( isset( $g_cache_dwg_tags[(int)$p_bug_id] ) ) {
			unset( $g_cache_dwg_tags[(int)$p_bug_id] );
		}
	}
}
 ...
 */
function tag_dwg_exists( $p_tag_id ) {
	return ( tag_dwg_get( $p_tag_id ) !== false );
}
 ...
 */
function tag_dwg_ensure_exists( $p_tag_id ) {
 ...
 */
function tag_dwg_is_unique( $p_name ) {
	$c_name = trim( $p_name );
 ...
 * @return void
 */
function tag_dwg_ensure_unique( $p_name ) {
	if( !tag_dwg_is_unique( $p_name ) ) {
 ...
 */
function tag_dwg_name_is_valid( $p_name, array &$p_matches, $p_prefix = '' ) {
 ...
 * @return void
 */
function tag_dwg_ensure_name_is_valid( $p_name ) {
	$t_matches = array();
 ...
 */
function tag_dwg_cmp_name( array $p_tag1, array $p_tag2 ) {
 ...
 */
function tag_dwg_parse_string( $p_string ) {
	$t_tags = array();
	...
	foreach( $t_strings as $t_name ) {
		...
		$t_matches = array();
		$t_tag_row = tag_dwg_get_by_name( $t_name );
		if( $t_tag_row !== false ) {
			$t_tags[] = $t_tag_row;
		} else {
			if( tag_dwg_name_is_valid( $t_name, $t_matches ) ) {
				$t_id = -1;
			} else {
				$t_id = -2;
			}
 ...
 *                    The array elements follow the tag_dwg_parse_string() format.
 * @throws ClientException
 */
function tag_dwg_attach_many( $p_bug_id, $p_tag_string, $p_tag_id = 0 ) {
	# If no work, then there is no need to do access check.
	if( $p_tag_id === 0 && is_blank( $p_tag_string ) ) {
		return true;
	}

	access_ensure_dwg_level( config_get( 'tag_attach_threshold' ), $p_bug_id );

	$t_tags = tag_dwg_parse_string( $p_tag_string );
	$t_can_create = tag_dwg_can_create();
	...
	if( 0 < $p_tag_id && tag_dwg_exists( $p_tag_id ) ) {
		$t_tags_attach[] = tag_dwg_get( $p_tag_id );
	}
	...
	foreach( $t_tags_create as $t_tag_row ) {
		$t_tag_row['id'] = tag_dwg_create( $t_tag_row['name'] );
		$t_tags_attach[] = $t_tag_row;
	}
 ...
 */
function tag_dwg_parse_filters( $p_string ) {
	$t_tags = array();
	$t_prefix = '[+-]{0,1}';
	...
	foreach( $t_strings as $t_name ) {
		...
		if( !is_blank( $t_name ) && tag_dwg_name_is_valid( $t_name, $t_matches, $t_prefix ) ) {
			$t_tag_row = tag_dwg_get_by_name( $t_matches[1] );
			if( $t_tag_row !== false ) {
 ...
 * @return ADORecordSet|bool Tags sorted by name, or false if the query failed.
 */
function tag_dwg_get_unused( $p_name_filter, $p_count, $p_offset ) {
	$t_where = '';
	$t_where_params = array();
 ...
 * @return int
 */
function tag_dwg_count( $p_name_filter, $p_unused = false ) {
	$t_where = '';
	$t_where_params = array();
 ...
 */
function tag_dwg_get_name( $p_tag_id ) {
	$t_tag_row = tag_dwg_get( $p_tag_id );
	if( $t_tag_row === false ) {
		return '';
	}
 ...
 */
function tag_dwg_get_by_name( $p_name ) {
	db_param_push();
 ...
 */
function tag_dwg_get_field( $p_tag_id, $p_field_name ) {
	$t_row = tag_dwg_get( $p_tag_id );
 ...
 */
function tag_dwg_can_create( $p_user_id = null ) {
 ...
 * @return void
 */
function tag_dwg_ensure_can_create( $p_user_id = null ) {
 ...
 * @throws ClientException
 */
function tag_dwg_create( $p_name, $p_user_id = null, $p_description = '' ) {
 ...
 * @throws ClientException
 */
function tag_dwg_update( $p_tag_id, $p_name, $p_user_id, $p_description ) {
	$t_tag_row = tag_dwg_get( $p_tag_id );
 ...
 * @throws ClientException
 */
function tag_dwg_delete( $p_tag_id ) {
 ...
 */
function tag_dwg_get_candidates_for_bug( $p_bug_id ) {
	db_param_push();
...
function tag_dwg_bug_get_row( $p_tag_id, $p_bug_id ) {
	...
	if( !isset( $g_cache_dwg_tags[$c_bug_id] ) ) {
		tag_dwg_cache_bug_tag_rows( array( $c_bug_id ) );
	}

	$t_bug_tags = $g_cache_dwg_tags[$c_bug_id];
...
function tag_dwg_bug_get_attached( $p_bug_id ) {
	...
	foreach( $t_bug_tags as $t_row ) {
		$t_tag_data = tag_dwg_get($t_row['tag_id']);
...
function tag_dwg_bug_detach( $p_tag_id, $p_bug_id, $p_add_history = true, $p_user_id = null ) {
	...
	if( !access_has_dwg_level( $t_detach_level, $p_bug_id, $t_user_id ) ) {
		throw new ClientException(
 ...
 * @return string
 */
function tag_dwg_get_link( array $p_tag_row ) {
...
function tag_dwg_display_link( array $p_tag_row, $p_bug_id = 0 ) {
	...
	if( $p_bug_id > 0 && access_has_dwg_level( $t_detach, $p_bug_id ) ) {
 ...
 */
function tag_dwg_stats_related( $p_tag_id, $p_limit = 5 ) {
	...
	while( $t_row = $t_query->fetch() ) {
		$t_tag_row = tag_dwg_get( $t_row['tag_id'] );
