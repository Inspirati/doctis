 ...
 * - Creator is optional, it is defaulted to logged in user.
 ...
 */
class DwgNoteAddCommand extends Command {
	/**
	...
	function validate() {
		...
		$this->issue = dwg_get( $t_issue_id, true );
		...
		if( empty( $this->text ) &&
		    ...
		    count( $this->files ) == 0 ) {
			throw new ClientException( 'Document note not specified.', ERROR_EMPTY_FIELD, array( lang_get( 'dwgnote' ) ) );
		}
		...
		if( !access_has_dwg_level( config_get( 'add_bugnote_threshold' ), $t_issue_id, $this->reporterId ) ) {
			throw new ClientException( "Creator can't add notes", ERROR_ACCESS_DENIED );
		}

		# Can reporter add private notes?
		if( $this->private ) {
			if( !access_has_dwg_level( config_get( 'set_view_status_threshold' ), $t_issue_id, $this->reporterId ) ) {
				throw new ClientException( "Creator can't add private notes", ERROR_ACCESS_DENIED );
			}
		}

		# Can reporter attach files, if supplied?
		if( $t_files_included ) {
			if( !file_dwg_allow_dwg_upload( $this->issue->id, $this->reporterId ) ) {
		...
		if( $t_time_tracking_mins > 0 ) {
			...
			if ( !access_has_dwg_level( config_get( 'time_tracking_edit_threshold' ), $t_issue_id, $this->reporterId ) ) {
	...
	protected function process() {
		...
		$t_file_infos = file_dwg_attach_files( $this->issue->id, $this->files, $t_note_id );

		# Process the mentions in the added note
		$t_user_ids_that_got_mention_notifications = dwgnote_process_mentions( $this->issue->id, $t_note_id, $this->payload( 'text' ) );
		...
		if( config_get( 'reassign_on_feedback' ) &&
			...
			if( $this->issue->handler_id !== NO_USER ) {
				...
				dwg_set_field( $this->issue->id, 'status', config_get( 'dwg_submit_status' ) );
			}
		}
		...
		email_dwgnote_add( $t_note_id, $t_file_infos, /* user_exclude_ids */ $t_user_ids_that_got_mention_notifications );

		# Event integration
		event_signal( 'EVENT_DWGNOTE_ADD', array( $this->issue->id, $t_note_id, $t_file_infos ) );
	...
	private function parseViewState() {
		...
		if( $t_view_state === null ) {
			$t_view_state = config_get( 'default_dwgnote_view_status' );
		} else {
