 ...
 * @subpackage classes
 */
class DwgMonitorTimelineEvent extends TimelineEvent {
	private $issue_id;
	private $monitor;
	...
	public function html() {
		$t_string = $this->monitor ? lang_get( 'timeline_document_monitor' ) : lang_get( 'timeline_document_unmonitor' );

		$t_html = $this->html_start( 'fa-eye' );
		$t_html .= '<div class="action">' . sprintf( $t_string, prepare_user_name( $this->user_id ), string_get_dwg_view_link( $this->issue_id ) ) . '</div>';
