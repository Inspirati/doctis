...
/**
 * A command that adds user to monitor a document.
 */
class DwgMonitorAddCommand extends Command {
	/**
	 * @var integer
	 */
	private $projectId;
	...
	/**
	 * @var array array of user ids to add to monitor list of the document
	 */
	...
	function validate() {		
		...
		$this->projectId = dwg_get_field( $t_issue_id, 'project_id' );
		...
		foreach( $t_user_ids as $t_user_id ) {
			...
			if( user_is_anonymous( $t_user_id ) ) {
				throw new ClientException( "anonymous account can't monitor documents", ERROR_PROTECTED_ACCOUNT );
			}
			...
			if( $t_logged_in_user != $t_user_id ) {
				...
				if( !access_has_dwg_level( $t_access_level, $t_issue_id, $t_logged_in_user ) ) {
	...
	protected function process() {
		...
		foreach( $this->userIdsToAdd as $t_user_id ) {
			dwg_monitor( $this->query( 'issue_id' ), $t_user_id );
		}
