...
class DwgRelationshipData {
	...
	/**
	 * @var int Destination Bug id
	 */
	public $dest_dwg_id;

	/**
	 * @var int Destination project id
	 */
...
$g_relationships[DWG_DEPENDANT] = array(
	'#forward' => true,
	'#complementary' => DWG_BLOCKS,
	'#name' => 'parent-of',
	'#description' => 'dependant_on',
	'#notify_added' => 'email_notification_title_for_action_dwg_dependant_on_relationship_added',
	'#notify_deleted' => 'email_notification_title_for_action_dwg_dependant_on_relationship_deleted',
	'#edge_style' => array(
		'color' => '#C00000',
		'dir' => 'back',
	),
);
$g_relationships[DWG_BLOCKS] = array(
	'#forward' => false,
	'#complementary' => DWG_DEPENDANT,
	'#name' => 'child-of',
	'#description' => 'blocks',
	'#notify_added' => 'email_notification_title_for_action_dwg_blocks_relationship_added',
	'#notify_deleted' => 'email_notification_title_for_action_dwg_blocks_relationship_deleted',
	'#edge_style' => array(
		'color' => '#C00000',
		'dir' => 'forward',
	),
);
$g_relationships[DWG_DUPLICATE] = array(
	'#forward' => true,
	'#complementary' => DWG_HAS_DUPLICATE,
	'#name' => 'duplicate-of',
	'#description' => 'duplicate_of',
	'#notify_added' => 'email_notification_title_for_action_dwg_duplicate_of_relationship_added',
	'#notify_deleted' => 'email_notification_title_for_action_dwg_duplicate_of_relationship_deleted',
	'#edge_style' => array(
		'style' => 'dashed',
		'color' => '#808080',
	),
);
$g_relationships[DWG_HAS_DUPLICATE] = array(
	'#forward' => false,
	'#complementary' => DWG_DUPLICATE,
	'#name' => 'has-duplicate',
	'#description' => 'has_duplicate',
	'#notify_added' => 'email_notification_title_for_action_dwg_has_duplicate_relationship_added',
	'#notify_deleted' => 'email_notification_title_for_action_dwg_has_duplicate_relationship_deleted',
);
$g_relationships[DWG_RELATED] = array(
	'#forward' => true,
	'#name' => 'related-to',
	'#complementary' => DWG_RELATED,
	'#description' => 'related_to',
	'#notify_added' => 'email_notification_title_for_action_dwg_related_to_relationship_added',
	'#notify_deleted' => 'email_notification_title_for_action_dwg_related_to_relationship_deleted',
);
 ...
 */
function dwg_relationship_type_ensure_valid( $p_relationship_type ) {
 ...
 */
function dwg_relationship_get_complementary_type( $p_relationship_type ) {
	global $g_relationships;
 ...
 * @internal
 */
function dwg_relationship_prepare_for_assignment( $p_src_bug_id, $p_dest_bug_id, $p_relationship_type ) {
	global $g_relationships;
	...
	if( $g_relationships[$p_relationship_type]['#forward'] === false ) {
		...
		$t_data = array(
			(int)$p_dest_bug_id,
			(int)$p_src_bug_id,
			dwg_relationship_get_complementary_type( $p_relationship_type )
		);
	} else {
		$t_data = array(
			(int)$p_src_bug_id,
 ...
 * @param bool $p_email_for_source  Should an email be triggered for source document?
 *
...
function dwg_relationship_add( $p_src_bug_id, $p_dest_bug_id, $p_relationship_type, $p_email_for_source = true ) {
	...
	email_dwg_relationship_added( $p_src_bug_id, $p_dest_bug_id, $p_relationship_type, $p_email_for_source );
 ...
 * @return int The new bug relationship id.
 */
function dwg_relationship_upsert( $p_src_bug_id, $p_dest_bug_id, $p_relationship_type, $p_email_for_source = true ) {
	# Check if there is other relationship between the bugs.
	$t_id_relationship = dwg_relationship_same_type_exists( $p_src_bug_id, $p_dest_bug_id, $p_relationship_type );

	if( $t_id_relationship > 0 ) {
		dwg_relationship_update( $t_id_relationship, $p_src_bug_id, $p_dest_bug_id, $p_relationship_type, $p_email_for_source );
		$t_relationship_id = $t_id_relationship;
	} else if( $t_id_relationship != -1 ) {
		$t_relationship_id = dwg_relationship_add( $p_src_bug_id, $p_dest_bug_id, $p_relationship_type, $p_email_for_source );
	} else {
		# else relationship is -1 - same type exists
		$t_relationship_id = dwg_relationship_exists( $p_src_bug_id, $p_dest_bug_id );
	}
 ...
 */
function dwg_relationship_delete( $p_relationship_id, $p_send_email = true ) {
	...
	$t_src_bug_id = $t_relationship->src_dwg_id;
	$t_dest_bug_id = $t_relationship->dest_dwg_id;
	...
	if( $p_send_email ) {
		email_dwg_relationship_deleted( $t_src_bug_id, $t_dest_bug_id, $t_rel_type );
	}
}
 ...
 */
function dwg_relationship_delete_all( $p_bug_id ) {
	$t_is_different_projects = false;
	$t_relationships = dwg_relationship_get_all( $p_bug_id, $t_is_different_projects );
	foreach( $t_relationships as $t_relationship ) {
		dwg_relationship_delete( $t_relationship->id, /* send_email */ false );
	}
}
...
function dwg_relationship_copy_all( $p_bug_id, $p_new_bug_id ) {
	...
	foreach( $t_relationships as $t_relationship ) {
		dwg_relationship_add(
			$p_new_bug_id,
			$t_relationship->src_dwg_id,
			dwg_relationship_get_complementary_type( $t_relationship->type ),
			/* email_for_source */ false );
	}
}
...
function dwg_relationship_get( $p_relationship_id ) {
	...
	if( $t_relationship ) {
		...
		$t_bug_relationship_data->src_dwg_id = $t_relationship['source_dwg_id'];
		$t_bug_relationship_data->dest_dwg_id = $t_relationship['destination_dwg_id'];
...
function dwg_relationship_get_all_src( $p_src_bug_id ) {
	...
	while( $t_row = db_fetch_array( $t_result ) ) {
		...
		$t_bug_relationship_data[$i]->src_dwg_id = $t_row['source_dwg_id'];
		$t_bug_relationship_data[$i]->src_project_id = $t_src_project_id;
		$t_bug_relationship_data[$i]->dest_dwg_id = $t_row['destination_dwg_id'];
	...
	if( !empty( $t_bug_array ) ) {
		dwg_cache_array_rows( $t_bug_array );
	}
...
function dwg_relationship_get_all( $p_bug_id, &$p_is_different_projects ) {
	$t_src = dwg_relationship_get_all_src( $p_bug_id );
	$t_dest = dwg_relationship_get_all_dest( $p_bug_id );
 ...
 * @param int $p_dest_bug_id Destination bug identifier.
 *
 * @return int Relationship ID if found, 0 otherwise.
 */
function dwg_relationship_exists( $p_src_bug_id, $p_dest_bug_id ) {
 ...
 */
function dwg_relationship_same_type_exists( $p_src_bug_id, $p_dest_bug_id, $p_rel_type ) {
	# Check if there is already a relationship set between them
	$t_id_relationship = dwg_relationship_exists( $p_src_bug_id, $p_dest_bug_id );

	if( $t_id_relationship > 0 ) {
		# The relationship exists, get all the info
		/** @noinspection PhpUnhandledExceptionInspection */
		$t_relationship = dwg_relationship_get( $t_id_relationship );

		if( $t_relationship->src_dwg_id == $p_src_bug_id && $t_relationship->dest_dwg_id == $p_dest_bug_id ) {
			if( $t_relationship->type == $p_rel_type ) {
				$t_id_relationship = -1;
			}
		} else {
			if( $t_relationship->type == dwg_relationship_get_complementary_type( $p_rel_type ) ) {
				$t_id_relationship = -1;
			}
		}
	}
...
/**
 * Retrieve the linked bug id of the relationship.
 *
 * If given the source bug Id return the destination bug Id, and vice-versa.
 *
 ...
 * @throws ClientException If the relationship does not exist.
 */
function dwg_relationship_get_linked_dwg_id( $p_relationship_id, $p_bug_id ) {
	$t_bug_relationship_data = dwg_relationship_get( $p_relationship_id );

	if( $t_bug_relationship_data->src_dwg_id == $p_bug_id ) {
		return $t_bug_relationship_data->dest_dwg_id;
	}

	if( $t_bug_relationship_data->dest_dwg_id == $p_bug_id ) {
		return $t_bug_relationship_data->src_dwg_id;
	}

	throw new ClientException(
		sprintf( "Unknown relationship '%d' for document '%d'", $p_relationship_id, $p_bug_id ),
		ERROR_RELATIONSHIP_NOT_FOUND
	);
}
 ...
 */
function dwg_relationship_get_description_src_side( $p_relationship_type ) {
	global $g_relationships;
	dwg_relationship_type_ensure_valid( $p_relationship_type );
 ...
 * @throws ClientException if the relationship type does not exist.
 */
function dwg_relationship_get_description_dest_side( $p_relationship_type ) {
	global $g_relationships;

	dwg_relationship_type_ensure_valid( $p_relationship_type );
	dwg_relationship_type_ensure_valid( $g_relationships[$p_relationship_type]['#complementary'] );
 ...
 * @throws ClientException if the relationship type does not exist.
 */
function dwg_relationship_get_description_for_history( $p_relationship_type ) {
	return dwg_relationship_get_description_src_side( $p_relationship_type );
}
 ...
 * @throws ClientException if the relationship type does not exist.
 */
function dwg_relationship_get_name_for_api( $p_relationship_type ) {
	global $g_relationships;
 ...
 */
function dwg_relationship_get_id_from_api_name( $p_relationship_type_name ) {
	global $g_relationships;
 ...
 *
 * N.B. we don't check if the parent bug is read-only. This is because the
 * answer of this function is independent from the state of the parent bug
 * itself.
 *
 * @param int $p_bug_id A bug identifier.
 *
...
function dwg_relationship_can_resolve_dwg( $p_bug_id ) {
	...
	foreach( $t_relationships as $t_relationship ) {
		...
		if( $t_relationship->type == DWG_DEPENDANT ) {
			$t_status = dwg_get_field( $t_relationship->dest_dwg_id, 'status' );
