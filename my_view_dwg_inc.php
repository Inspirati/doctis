...
if( !defined( 'MY_VIEW_DWG_INC_ALLOW' ) ) {
	return;
}
...
$t_recent_days = 30;
$c_filter['recent_mod'] = filter_dwg_create_recently_modified( $t_recent_days );
...
$t_url_link_parameters['feedback'] = [
	FILTER_PROPERTY_CREATOR_ID => $t_current_user_id,
	FILTER_PROPERTY_STATUS => config_get( 'dwg_feedback_status' ),
	FILTER_PROPERTY_HIDE_STATUS => $t_hide_status_default,
];
...
$t_rows = filter_dwg_get_dwg_rows( $f_page_number, $t_per_page, $t_page_count, $t_bug_count, $c_filter[$t_box_title] );
...
dwg_cache_columns_data( $t_rows , array( 'attachment_count' ) );
...
$t_bug_string = $t_bug_count == 1 ? 'document' : 'documents';
...
print_hyperlink( $t_box_url, $t_box_title_label, false, 'white' );
...
for( $i = 0;$i < $t_count; $i++ ) {
	...
	if( ( file_dwg_can_view_dwg_attachments( $t_bug->id, null ) ) ) {
		$t_attachment_count = file_dwg_attachment_count( $t_bug->id );
	}
...
<tr class="my-buglist-bug <?php echo $t_bug_class?>">
	...
	<td class="nowrap width-13 my-buglist-id">
		<?php
			...
			$t_can_update = !dwg_is_readonly( $t_bug->id ) &&
				access_has_dwg_level( config_get( 'update_dwg_threshold', null, $t_current_user_id, $t_bug->project_id  ), $t_bug->id );
			if( $t_can_update ) {
				echo '<a class="edit" href="' . string_get_dwg_update_url( $t_bug->id ) . '">';
				print_icon( 'fa-pencil', 'bigger-130 padding-2 grey', lang_get( 'edit' ) );
				echo '</a>';
			}

			if( ON == config_get( 'show_priority_text' ) ) {
				print_dwg_formatted_priority_string( $t_bug );
			} else {
				print_status_icon( $t_bug->priority );
			}

			if( $t_attachment_count > 0 ) {
				$t_href = string_get_dwg_view_url( $t_bug->id ) . '#attachments';
	...
	<td>
		<?php
			if( ON == config_get( 'show_dwg_project_links' ) && helper_get_current_project() != $t_bug->project_id ) {
				echo '<span>[', string_display_line( project_get_name( $t_bug->project_id ) ), '] </span>';
			}
			$t_bug_url = string_get_dwg_view_url( $t_bug->id );
			// echo '<span><a href="' . $t_bug_url . '">' . $t_summary . '</a></span><br />';
			echo '<span><a href="' . $t_bug_url . '">' . $t_bug->title . '</a></span><br />';
	?>
		<?php
