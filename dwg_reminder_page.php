...
<form method="post" action="dwg_reminder.php">
<?php echo form_security_field( 'dwg_reminder' ) ?>
<input type="hidden" name="dwg_id" value="<?php echo $f_bug_id ?>" />
<div class="widget-box widget-color-blue2">
<div class="widget-header widget-header-small">
	<h4 class="widget-title lighter">
		<?php print_icon( 'fa-envelope', 'ace-icon' ); ?>
		<?php echo lang_get( 'dwg_reminder' ) ?>
	</h4>
</div>
...
?>
	<tr>
		...
		<td>
			<select id="recipient" name="to[]" multiple="multiple" size="9" class="width-100">
				<?php
				$t_project_id = dwg_get_field( $f_bug_id, 'project_id' );
				$t_access_level = config_get( 'reminder_receive_threshold' );
				if( $t_bug->view_state === VS_PRIVATE ) {
					$t_private_bug_threshold = config_get( 'private_dwg_threshold' );
		...
		<div class="widget-toolbox padding-8 clearfix">
			<input type="submit" class="btn btn-primary btn-white btn-round" value="<?php echo lang_get( 'dwg_send_button' ) ?>" />
		</div>
	</div>
	</div>
	</form>
	...
	<div class="alert alert-info">
		...
		<?php
			...
			echo ' ' . lang_get( 'dwg_reminder_explain' ) . ' ';
			if( ON == config_get( 'reminder_recipients_monitor_dwg' ) ) {
				echo lang_get( 'dwg_reminder_monitor' ) . ' ';
			}
			if( $t_store_reminders ) {
				echo lang_get( 'dwg_reminder_store' );
			}

			if( mention_enabled() ) {
				echo '<br /><br />', sprintf( lang_get( 'dwg_reminder_mentions' ), '<strong>' . mentions_tag() . user_get_username( auth_get_current_user_id() ) . '</strong>' );
			}
		?>
		</p>
	</div>
...
include( $t_mantis_dir . 'dwg_view_inc.php' );
