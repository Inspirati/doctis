 ...
 * @subpackage classes
 */
class DwgTagTimelineEvent extends TimelineEvent {
	private $issue_id;
	private $tag_name;
	private $tag;
	...
	public function html() {
		$t_string = $this->tag ? lang_get( 'timeline_document_tagged' ) : lang_get( 'timeline_document_untagged' );
		$t_tag_row = tag_dwg_get_by_name( $this->tag_name );
		...
		$t_html .= '<div class="action">'
			. sprintf(
				...
				$t_tag_row ? tag_dwg_get_link( $t_tag_row ) : $this->tag_name
			)
			. '</div>';
